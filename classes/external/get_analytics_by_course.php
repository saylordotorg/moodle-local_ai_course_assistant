...
/**
 * Get analytics data broken down by course.
 *
 * Returns overview stats for each course that has SOLA conversation data.
 *
...
class get_analytics_by_course extends external_api {
    ...
    public static function execute(int $courseid = 0, int $since = 0): array {
        ...
        // Get list of courses with SOLA data.
        if ($courseid > 0) {
            $courseids = [$courseid];
        } else {
            $sql = "SELECT DISTINCT courseid FROM {local_ai_course_assistant_msgs}";
        ...
        foreach ($courseids as $cid) {
            ...
            $overview = analytics::get_overview((int) $cid, $since);
            $daily = analytics::get_daily_usage((int) $cid);
            ...
            if (method_exists(analytics::class, 'get_enrollment_counts')) {
                $coursedata['enrollment'] = analytics::get_enrollment_counts((int) $cid, $since);
            }
            if (method_exists(analytics::class, 'get_session_stats')) {
                $coursedata['sessions'] = analytics::get_session_stats((int) $cid, $since);
            }
            if (method_exists(analytics::class, 'get_return_rate')) {
                $coursedata['return_rate'] = analytics::get_return_rate((int) $cid, $since);
            }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'data' => new external_value(PARAM_RAW, 'JSON-encoded per-course analytics data'),
        ]);
    }
}
