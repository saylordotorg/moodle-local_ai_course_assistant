 ...
 *
 * Returns counts of different usage types (e.g., questions, quiz, study plan).
 *
...
class get_analytics_usage_types extends external_api {
    ...
    public static function execute(int $courseid = 0, int $since = 0): array {
        ...
        if (method_exists(analytics::class, 'get_usage_type_breakdown')) {
            $result = analytics::get_usage_type_breakdown($courseid, $since);
        } else {
            // Fall back to common prompts as a proxy.
            if ($courseid > 0) {
                $result = analytics::get_common_prompts($courseid, $since);
            }
        }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'data' => new external_value(PARAM_RAW, 'JSON-encoded usage type breakdown'),
        ]);
    }
}
