...
**Two reported UI bugs, both from staging. No database changes, no new settings,
no i18n additions.**
...
JS, template and test changes only. No `db/upgrade.php` step runs, no setting is
written, and no credential-bearing setting is touched -- so the upgrade adds no
rows to `config_log`. A cache purge is all that is required after deploy, and a
revert is a straight redeploy of the previous tag.

### Testing

- Full suite: 1,510 tests / 0 failures / 1 skipped.
- Validators: 36/36.
- Page smoke: 35/35.
