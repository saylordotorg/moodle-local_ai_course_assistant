...
4. **The rerank GO numbers are raw-cosine recall — and the 0.25 floor is now confirmed non-binding.** The fixture benchmark that drives the Voyage rerank-2.5 decision (§10 / companion `sola-rag-fixture-benchmark-2026-06-10.md`) ranks by cosine over all chunks and does not itself apply the v6.2.0 production floor (0.25) or current-page bias. Concern raised + **measured on dev.sylr.org 2026-06-13**: across all 40 labeled fixtures the worst target chunk scores **0.3153** cosine (mean 0.4862), so **0 of 40** fall under the 0.25 floor — the floor drops none of the known-good chunks, and the +17.5pp recall@3 rerank lift holds for the production retrieval path, not just raw cosine. No floor change needed; the rerank GO stands as a production number. (~0.065 headroom below the worst target — re-check if the corpus is re-indexed.)
