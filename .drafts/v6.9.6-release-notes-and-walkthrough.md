...
**The `rag_topk` default changes from 5 to 3 — but most existing sites will not pick it up.**

A new default only takes effect where the setting has never been written to `config_plugins`, and Moodle persists defaults there the first time anyone saves the settings page. Any site whose admin has opened and saved SOLA settings already has a stored `5` and keeps it. In practice: **new installs get 3; existing sites keep whatever is stored and need the change applied by hand.**

This was found by verifying the release on the dev fleet rather than assuming. All five dev sites reported `rag_topk=5` after upgrading to 6.9.6, with `EXISTS value=5` in `config_plugins`; they were moved to 3 manually. Both production sites have run 3 since 2026-08-04.

To check your own: Site administration → Plugins → Local plugins → SOLA → Retrieval. If it reads 5, that value is stored and this release did not change it.

The evidence for 3 is a blind A/B over 40 questions, order alternated and judged by Sonnet 4.6, which found the two settings statistically tied: topk=3 won 11, topk=5 won 12, and 17 tied — while topk=3 used 31% fewer prompt tokens. A cost saving, not a quality trade.
...
If it reads 3, nothing needs doing. **If it reads 5, this release did not change it** — the value is stored in `config_plugins` and a new default cannot override a stored value. That is the common case, not the exception: Moodle writes the default to the database the first time anyone saves the settings page, so most sites that have ever been configured are in it. Set it to 3 and save; you should see roughly 31% fewer prompt tokens with no measurable quality change.

For a fleet, the CLI is faster than the settings page and invalidates the config cache correctly:

```
php admin/cli/cfg.php --component=local_ai_course_assistant --name=rag_topk --set=3
