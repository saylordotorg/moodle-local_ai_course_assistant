...
To: Artem / Catalyst
Subject: SOLA plugin upgrade request: v5.4.5 to v6.5.0 on Degrees, then Learn
...
**What:** upgrade the plugin submodule pin from v5.4.5 to tag v6.5.0
(https://github.com/saylordotorg/moodle-local_ai_course_assistant/releases/tag/v6.5.0).
...
**Release quality:** every tag ships only after our automated gate: validator and jailbreak suites, about 500 PHPUnit tests, Behat, an accessibility audit, CI green on the tagged commit, and deployment plus smoke verification on our five dev sites. v6.5.0 has been through all of it.
...
- Runbook with full detail: `.drafts/sola-prod-upgrade-runbook-v5.4.5-to-v6.5.0.md` (settings deltas, behavior changes, post deploy verification checklist, first week config steps).
- Post deploy verification (Tom or Claude runs it): plugin version shows 2026061012/6.5.0; BUS101 chat round trip streams with SOLA_NEXT chips; history shows no telemetry rows; analytics + token analytics render; emergency panel shows all flags inactive.
...
- Why v6.5.0 and not v6.3.0: same tested migration path (the only db/ change after v6.3.0 is a task registration; nothing schema-touching in v6.4.2-v6.5.0), and it carries the v6.0.1 security fix, the v6.2.x RAG and CSP fixes, the policy bundle mechanism that reduces future Catalyst engagements, and the avatar engagement probe.
