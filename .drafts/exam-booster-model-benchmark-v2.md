...
1. **Qwen-vs-Opus reviewer flag overlap.** Take the 15 Qwen flags and the 15 Opus flags on the same Sonnet-4.6 generated exam, compute the overlap, and decide whether Qwen is signal or noise. Cost: ~$0.02, time: ~2 minutes. **[2026-06-09 — RESOLVED]** Ran on the targeted re-run's Gemini-generated base (Sonnet output from v2 isn't on disk; the qualitative question is base-independent). Opus: 15 flags / $0.28 / 41s. Qwen: 5 flags / $0.00 / 29s. **Overlap: 2 items (1d.1, 1g.1). Jaccard: 0.11.** Qwen flagged 3 items Opus passed (1a.1, 1b.1, 1c.1, all Unit-1 clustered); Opus flagged 13 items Qwen missed, spread across Units 1–5. Translation: Qwen at 7B is producing its own short flag list, not catching the same items Opus does. Keep Qwen in the panel as a "free signal" — the 2 overlap hits + 3 unique hits cost nothing — but treat its output as a supplement, not a substitute for the strong reviewers. Caveat: single Gemini-output base; a 2nd–3rd repetition on different base inputs would tighten the conclusion.
