...
$string['settings:default_course_mode_desc'] = 'Управляет тем, отображается ли [[tutorshort]] на курсе, когда выбор для конкретного курса не сделан. Новые установки по умолчанию имеют значение "Отключено по умолчанию", чтобы администраторы могли включать курс за курсом со страницы Analytics или страницы Course AI Settings.';
...
$string['coursesettings:sola_enabled'] = '[[tutorshort]] на этом курсе';
$string['coursesettings:sola_enabled_toggle'] = 'Показывать виджет [[tutorshort]] на этом курсе';
$string['coursesettings:sola_enabled_desc'] = 'Управляет тем, отображается ли виджет чата [[tutorshort]] на этом курсе. Значение по умолчанию для всего сайта устанавливается в настройках плагина в разделе General > Default for new courses.';
...
$string['chat:greeting'] = 'Привет, {$a}! Я [[tutorshort]]. Чем могу помочь сегодня?';
$string['chat:title'] = '[[tutorshort]]';
$string['chat:assistant'] = '[[tutorshort]]';
$string['chat:open'] = 'Открыть [[tutorshort]]';
...
$string['chat:voice_speaking'] = '[[tutorshort]] говорит...';
$string['chat:voice_idle'] = 'Готово';
$string['chat:voice_error'] = 'Не удалось подключиться к голосовому режиму. Проверьте настройки.';
$string['chat:quiz_locked'] = '[[tutorshort]] приостановлена во время тестов для поддержания академической честности. Удачи!';
...
$string['settings:cdn_heading_desc'] = 'Раздавайте фронтенд-ресурсы [[tutorshort]] (JS/CSS) с внешнего CDN вместо файловой системы Moodle. Это позволяет обновлять фронтенд без обновления плагина. Оставьте URL CDN пустым для использования локальных файлов плагина.';
...
$string['chat:voice_title'] = 'Поговорите с [[tutorshort]]';
...
$string['mobile_disabled'] = '[[tutorshort]] недоступен для этого курса.';
$string['mobile_placeholder'] = 'Задайте вопрос...';
$string['mobile_welcome'] = 'Привет, {$a}!';
$string['mobile_welcome_sub'] = 'Я [[tutorshort]], ваш помощник по обучению. Чем могу помочь?';
...
$string['settings:rag_return_scope'] = 'Область возврата результатов поиска';
$string['settings:rag_return_scope_desc'] = 'Что добавлять в промпт для каждого найденного совпадения. "chunk" (по умолчанию) добавляет найденный фрагмент длиной около 400 слов. "window" добавляет найденный фрагмент плюс соседние фрагменты с той же страницы. "page" добавляет всю страницу модуля, к которой относится совпадение. window/page полезны, когда ответ охватывает всю страницу, но за счёт увеличения размера промпта.';
$string['settings:rag_window_size'] = 'Размер окна поиска';
$string['settings:rag_window_size_desc'] = 'Для области возврата "window": количество соседних фрагментов, включаемых с каждой стороны найденного фрагмента. По умолчанию 1.';
$string['settings:rag_parent_max_chars'] = 'Лимит родительского фрагмента (символы)';
$string['settings:rag_parent_max_chars_desc'] = 'Максимальное количество символов для расширенного фрагмента window/page. Страницы, превышающие лимит, откатываются к окну (window), а затем к одному фрагменту. По умолчанию 6000.';
...
$string['demo:course_fullname'] = 'Тестовый курс [[tutorshort]] (скрытый)';
...
$string['chat:consent_body'] = '{$a->product} — это учебный ассистент на базе ИИ. Ваши сообщения и ответы {$a->product} хранятся в базе данных Moodle учреждения {$a->institution}, а десять последних ходов отправляются утверждённому поставщику модели ИИ для ответа на ваши вопросы. Ваше имя передаётся для персонализации; никакая другая идентифицирующая информация поставщику ИИ не отправляется. Если вы запросите помощь человека и ваш вопрос будет эскалирован, этот разговор (включая ваше имя и адрес электронной почты) может быть передан нашей команде поддержки. Вы можете в любой момент скачать, удалить или прекратить использование {$a->product}.';
...
$string['settings:xai_proxy_url_desc'] = 'Публичный URL wss сервиса прокси [[tutorshort]] xAI Realtime (например, wss://voice.example.com/xai-rt/rt). Если задан вместе с секретом JWT, голос xAI маршрутизируется через прокси, и главный API-ключ xAI никогда не попадает в браузер. Оставьте пустым, чтобы использовать прямое подключение (не рекомендуется для продакшена).';
...
$string['settings:current_page_content_maxchars_desc'] = 'Максимальное число символов текста текущей страницы, добавляемых в системный промпт как раздел «Current Page Content», когда RAG отключён. Значение по умолчанию 8,000 хорошо обосновывает вопросы по конкретной странице, оставляя при этом бюджет для структуры и инструкций. (При включённом RAG страница вместо этого обосновывается собственными наиболее релевантными фрагментами со смещением к текущей странице, поэтому это ограничение не применяется.) Очень длинная страница обрезается с начала до этого числа символов, поэтому конец чрезвычайно длинной страницы может не процитироваться; включение RAG позволяет этого избежать. Сайты, заботящиеся о расходах, могут ограничить значение ниже (например, 3,000-4,000). Ограничено диапазоном 500-8,000. Независимо от <code>prompt_budget_chars</code>: это ограничивает только раздел страницы; бюджет ограничивает весь промпт.';
...
$string['pedagogy:program_path'] = 'Учёт дальнейшего пути обучения включён по умолчанию';
$string['pedagogy:program_path_desc'] = 'Когда параметр включён, [[tutorshort]] может подсказать учащемуся, куда ведёт текущий курс дальше в рамках его программы (степени или сертификата) и как сегодняшние понятия связаны с последующими курсами. [[tutorshort]] читает данные плагина Moodle Programs (Degrees и Learn) и называет конкретный следующий курс только там, где программа задаёт предварительное условие или обязательный порядок прохождения; в остальных случаях указывает положение учащегося на пути обучения. Носит исключительно рекомендательный характер — никогда не меняет запись на курсы или уровень освоения и использует только данные о программе самого текущего учащегося. Если программа неприменима, ничего не делает без уведомлений.';

// Learning path map + next-course nudge (v5.9.0).
$string['pedagogy:learning_path'] = 'Карта учебного пути и подсказка о следующем курсе включены по умолчанию';
$string['pedagogy:learning_path_desc'] = 'Когда включено, [[tutorshort]] добавляет визуальную панель учебного пути (кнопка "мой путь" в заголовке виджета), показывающую программу учащегося как последовательность курсов, каждый из которых можно развернуть, чтобы увидеть его цели и уровень освоения учащимся. Когда учащийся достигает порога для текущего курса (завершение или высокая доля освоенных целей), [[tutorshort]] также показывает мягкий баннер "готов к следующему курсу" и упоминает об этом в беседе. Только рекомендательно; использует собственное распределение программы учащегося; ничего не делает незаметно там, где никакая программа не применяется.';
$string['settings:learning_path_mastery_threshold'] = 'Порог готовности учебного пути (%)';
$string['settings:learning_path_mastery_threshold_desc'] = 'Процент отслеживаемых целей курса, которые учащийся должен освоить, прежде чем подсказка учебного пути сочтёт его готовым к следующему курсу. Завершение курса в Moodle является другим триггером; подсказка срабатывает по тому, что произойдёт раньше. По умолчанию 80.';
$string['pathpanel_title'] = 'Мой учебный путь';
$string['pathpanel_open'] = 'Мой учебный путь';
$string['pathpanel_empty'] = 'Для этого курса пока нет доступного пути программы.';
$string['path_position'] = 'Курс {$a->index} из {$a->total}';
$string['path_status_done'] = 'Завершено';
$string['path_status_current'] = 'Вы здесь';
$string['path_status_upcoming'] = 'Предстоит';
$string['path_mastery_mastered'] = 'Освоено';
$string['path_mastery_in_progress'] = 'В процессе';
$string['path_mastery_not_started'] = 'Не начато';
$string['path_mastery_demonstrated_elsewhere'] = 'Показано в другом курсе';
$string['nudge_ready_title'] = 'Готовы двигаться дальше';
$string['nudge_ready_body'] = 'Отличная работа — вы готовы к {$a}.';
$string['nudge_view_path'] = 'Посмотреть мой путь';
$string['nudge_dismiss'] = 'Закрыть';

// v5.10.x strings (token-aware budgeting, backend retry, self-test, deployment presets, escalation consent, privacy).
$string['settings:backend_context_tokens'] = 'Контекстное окно бэкенда (токены)';
$string['settings:backend_context_tokens_desc'] = 'Максимальная длина контекста (max_model_len) вашего бэкенда ИИ в токенах. Установите 0 для размещённых моделей с большим окном (без ограничения). При значении выше 0 (например, 8192 для самостоятельно размещённого бэкенда vLLM) [[tutorshort]] уменьшает указанный выше бюджет символов системного промпта, чтобы промпт вместе с зарезервированным выводом и историей разговора умещался в окне, даже в языках с высокой плотностью токенов. Смотрите страницу вики Deployment Sizing, чтобы узнать, как это соотносится с числом одновременных пользователей.';
$string['settings:backend_retry_attempts'] = 'Попытки повтора бэкенда';
$string['settings:backend_retry_attempts_desc'] = 'Сколько раз повторять временную ошибку бэкенда (HTTP 429 или 503) перед показом ошибки учащемуся. Повторы происходят только до того, как был передан какой-либо текст ответа, поэтому вывод никогда не дублируется. Предназначено для небольших самостоятельно размещённых бэкендов, которые отклоняют запросы под нагрузкой. Установите 0 для отключения. По умолчанию 2.';
$string['settings:backend_retry_max_wait'] = 'Максимальное время ожидания повтора бэкенда (секунды)';
$string['settings:backend_retry_max_wait_desc'] = 'Верхняя граница в секундах того, как долго соблюдать заголовок Retry-After от бэкенда перед повтором. Когда бэкенд не отправляет Retry-After, [[tutorshort]] вместо этого использует короткую экспоненциальную задержку. По умолчанию 5.';
$string['prompt:truncation_hint'] = 'ПРИМЕЧАНИЕ. Полное содержимое курса не удалось просмотреть в этом ходе из-за ограничений длины. Если учащийся спрашивает о чём-то, что вы не можете найти в предоставленном содержимом, скажите, что вы не смогли просмотреть весь курс, и предложите ему открыть конкретную страницу или активность, где раскрывается эта тема, вместо того чтобы утверждать, что она отсутствует в курсе.';
$string['selftest:title'] = 'Самопроверка бэкенда';
$string['selftest:intro'] = 'Запустите живую проверку настроенного бэкенда ИИ: небольшой обмен сообщениями в чате, автоматическое определение контекстного окна (max_model_len) и сравнение с вашей настройкой контекстного окна бэкенда, нижний порог бюджета системного промпта и (когда RAG включён) обмен встраиванием. Сетевые вызовы выполняются только при нажатии «Запустить».';
$string['selftest:run'] = 'Запустить самопроверку бэкенда';
$string['selftest:check'] = 'Проверка';
$string['selftest:status'] = 'Статус';
$string['selftest:detail'] = 'Подробности';
$string['selftest:link'] = 'Страница самопроверки бэкенда';
$string['selftest:link_desc'] = 'Откройте страницу <a href="{$a}">Самопроверка бэкенда</a>, чтобы убедиться, что ваш бэкенд ИИ работает и правильно масштабирован. Полезно сразу после настройки самостоятельно размещённого бэкенда.';
$string['profile:title'] = 'Предустановки развёртывания';
$string['profile:intro'] = 'Примените рекомендуемый набор настроек для вашего типа развёртывания. Значения записываются в обычные настройки плагина и остаются индивидуально редактируемыми впоследствии. Применение предустановки перезаписывает перечисленные настройки.';
$string['profile:current'] = 'Последняя применённая предустановка: {$a}';
$string['profile:setting'] = 'Настройка';
$string['profile:value'] = 'Значение';
$string['profile:self_hosted_small'] = 'Самостоятельно размещённый малый контекст (один GPU, например A30 24GB / vLLM при 8K)';
$string['profile:hosted_large'] = 'Размещённый большой контекст (по умолчанию)';
$string['profile:apply_self_hosted_small'] = 'Применить предустановку самостоятельно размещённого малого контекста';
$string['profile:apply_hosted_large'] = 'Применить значения по умолчанию размещённого большого контекста';
$string['profile:applied'] = 'Предустановка {$a} применена. Значения теперь находятся в настройках вашего плагина.';
$string['profile:unknown'] = 'Неизвестная предустановка развёртывания.';
$string['profile:link'] = 'Страница предустановок развёртывания';
$string['profile:link_desc'] = 'Откройте страницу <a href="{$a}">Предустановки развёртывания</a>, чтобы применить рекомендуемый набор настроек для размещённого или самостоятельно размещённого бэкенда.';
$string['settings:zendesk_require_consent'] = 'Требовать согласие перед эскалацией в поддержку';
$string['settings:zendesk_require_consent_desc'] = 'Когда включено (рекомендуется), [[tutorshort]] эскалирует разговор в службу поддержки Zendesk только после того, как учащийся принял уведомление о согласии при первом запуске, в котором раскрывается, что запрос помощи человека передаёт разговор (включая имя и адрес электронной почты) в поддержку. Отключайте это только в том случае, если вы получаете это согласие иным способом; при отключённой настройке эскалации отправляются немедленно. Не действует, пока не включена эскалация Zendesk.';
$string['chat:escalation_needs_consent'] = 'Похоже, для этого требуется участник нашей команды поддержки. Чтобы передать это им, мне пришлось бы поделиться этим разговором, включая ваше имя и адрес электронной почты, со службой поддержки. Вы ещё не согласились на это, поэтому я ничего не отправил. Если вам нужна помощь человека, пожалуйста, примите уведомление о передаче данных для этого ассистента и спросите снова, или свяжитесь со службой поддержки напрямую.';
$string['privacy:metadata:email_optout'] = 'Настройки отказа от писем по получателю (от каких типов писем получатель отписался).';
$string['privacy:metadata:email_optout:email'] = 'Адрес электронной почты получателя, к которому применяется отказ.';
$string['privacy:metadata:email_optout:optout_type'] = 'Тип письма, от которого отказался получатель.';
$string['privacy:metadata:email_optout:userid'] = 'Пользователь Moodle, которому принадлежит отказ, когда известен.';
$string['chat:consent_scroll_hint'] = 'Пожалуйста, прокрутите вниз, чтобы прочитать уведомление полностью, прежде чем продолжить.';
$string['settings:rag_min_similarity'] = 'Минимальная релевантность (косинус)';
$string['settings:rag_min_similarity_desc'] = 'Отбрасывать извлечённые фрагменты, у которых косинусное сходство с вопросом ниже этого значения, чтобы вопрос не по теме или с малым содержанием добавлял меньше (или ноль) фрагментов вместо того, чтобы всегда дополнять до Top-K слабыми совпадениями. Диапазон от 0 до 1; 0 отключает фильтр. Подходящее значение зависит от модели эмбеддинга: 0.25 подходит для text-embedding-3-small. Повысьте его, чтобы быть строже (меньше контекста, более релевантного теме), понизьте, чтобы быть мягче.';
$string['settings:rag_currentpage_boost'] = 'Бонус текущей страницы';
$string['settings:rag_currentpage_boost_desc'] = 'Небольшой бонус, добавляемый к оценке релевантности фрагментов со страницы, которую учащийся просматривает в данный момент, чтобы вопросы вроде «объясни это» отдавали предпочтение видимой странице, когда оценки близки. Только порядок: это не заставляет нерелевантный фрагмент страницы пройти через фильтр минимальной релевантности. Установите 0, чтобы отключить.';
$string['settings:history_mode'] = 'Режим выбора истории';
$string['settings:history_mode_desc'] = 'Как выбираются прошлые реплики разговора перед отправкой в модель. <strong>Семантический</strong> сохраняет только недавние реплики, релевантные текущему вопросу (и всегда самый последний обмен), чтобы устаревшая, не относящаяся к теме предыдущая реплика не раздувала стоимость и не уводила ответ в сторону; он выполняет один дополнительный вызов эмбеддинга на сообщение. <strong>По давности</strong> сохраняет последние пары «Max Conversation History» независимо от релевантности (давно установленное поведение, без дополнительного вызова). Если эмбеддинг недоступен, семантический режим автоматически переключается на режим по давности.';
$string['settings:history_mode_semantic'] = 'Семантический (релевантные недавние реплики)';
$string['settings:history_mode_recency'] = 'По давности (последние N пар)';
$string['settings:history_semantic_minscore'] = 'Нижний порог релевантности истории (косинус)';
$string['settings:history_semantic_minscore_desc'] = 'В семантическом режиме истории прошлая реплика сохраняется только в том случае, если её сходство с текущим вопросом не ниже этого значения (самый последний обмен сохраняется всегда). Диапазон от 0 до 1; зависит от модели. Повысьте, чтобы быть строже (меньше истории), понизьте, чтобы сохранять больше.';
$string['settings:history_candidates'] = 'Окно кандидатов истории';
$string['settings:history_candidates_desc'] = 'В семантическом режиме истории на релевантность оцениваются только указанное число самых последних пар (ограничение стоимости). Пары старше этого окна не отправляются. Держите это значение равным «Max Conversation History» или выше.';

// v5.11.0 - v6.2.0 strings (added 2026-06-10).
$string['settings:embed_provider_voyage'] = 'Voyage AI (voyage-3.5 — рекомендовано; +4 MTEB по сравнению с OpenAI 3-small, 4x контекст, многоязычный)';
$string['settings:rerank_heading'] = 'RAG: Двухэтапный поиск (ре-ранкинг)';
$string['settings:rerank_heading_desc'] = 'Необязательный второй этап поиска: косинусное сходство выбирает top-N фрагментов-кандидатов (по умолчанию 50), затем cross-encoder ре-ранкер оценивает каждую пару (запрос, фрагмент), и лучшие top-K помещаются в подсказку. По умолчанию выключено; при отсутствии конфигурации ре-ранкера или его сбое переключается на одноэтапный косинусный поиск.';
$string['settings:rerank_enabled'] = 'Двухэтапный поиск (Voyage rerank-2.5)';
$string['settings:rerank_enabled_desc'] = 'При включении поиск RAG становится двухэтапным: косинусное сходство возвращает top-N кандидатов (по умолчанию 50), затем cross-encoder Voyage rerank-2.5 оценивает каждого и top-K попадают в подсказку. Опубликованные результаты: +15 Recall@10 enterprise, +39% NDCG BEIR. Тарификация ~$0,05/MTok. Требуется <code>rerank_apikey</code> ниже; при сбое ре-ранкинга или отсутствии конфигурации плавно переключается на одноэтапный косинусный поиск.';
$string['settings:rerank_apikey'] = 'Ключ API ре-ранкера';
$string['settings:rerank_apikey_desc'] = 'Ключ API Voyage AI для rerank-2.5. Оставьте пустым для повторного использования ключа API встраивания выше (типичные развёртывания Voyage используют один ключ для embed + rerank).';
$string['settings:rerank_model'] = 'Модель ре-ранкера';
$string['settings:rerank_model_desc'] = 'По умолчанию <code>rerank-2.5</code>. Здесь можно указать новые модели Voyage rerank.';
$string['settings:rerank_apibaseurl'] = 'Базовый URL API ре-ранкера';
$string['settings:rerank_apibaseurl_desc'] = 'Переопределяет базовый URL Voyage rerank. Оставьте пустым для использования базового URL API встраивания выше или стандартного Voyage (<code>https://api.voyageai.com/v1</code>).';
$string['settings:rerank_candidates'] = 'Окно кандидатов ре-ранкера';
$string['settings:rerank_candidates_desc'] = 'Сколько косинусных кандидатов top-N поступает на этап переранжирования. По умолчанию 20. Измерено на 1008 запросах: 20 не уступает 30 по полноте (R@3 89,0 % против 89,3 %) при затратах на треть меньше, а 50 стоит в 2,5 раза дороже без измеримого выигрыша. Ниже 10 полнота ухудшается. Затраты растут линейно с этим значением.';
$string['settings:stt_selfhosted_heading'] = 'Самостоятельно размещённая транскрипция (Whisper)';
$string['settings:stt_selfhosted_heading_desc'] = 'Запустите speech-to-text на собственном оборудовании без поминутной оплаты. Направьте [[tutorshort]] на любой совместимый с OpenAI сервер транскрипции: Docker <code>whisper-server</code>, <code>speaches</code> (faster-whisper) или сервер <code>whisper.cpp</code>. Когда здесь задан URL сервера, он становится путём STT по умолчанию; выберите платного провайдера в настройке Активный провайдер STT выше для переопределения. Если сервер находится в частной сети или использует plain http, добавьте его хост в список доверенных конечных точек SSRF в разделе Безопасность.';
$string['settings:stt_selfhosted_url'] = 'URL самостоятельно размещённого STT-сервера';
$string['settings:stt_selfhosted_url_desc'] = 'Базовый URL совместимого с OpenAI сервера транскрипции, например <code>http://10.0.0.5:8000</code>. [[tutorshort]] автоматически добавляет <code>/v1/audio/transcriptions</code>; также принимается полный путь к конечной точке. Оставьте пустым для отключения самостоятельно размещённого STT.';
$string['settings:stt_selfhosted_model'] = 'Модель самостоятельно размещённого STT';
$string['settings:stt_selfhosted_model_desc'] = 'Имя модели, передаваемое серверу, соответствующее загруженной модели Whisper — например <code>Systran/faster-whisper-small</code> для speaches или <code>large-v3</code>. Оставьте пустым для отправки <code>whisper-1</code>, который большинство самостоятельно размещённых серверов принимает или игнорирует.';
$string['settings:stt_selfhosted_apikey'] = 'Ключ API самостоятельно размещённого STT';
$string['settings:stt_selfhosted_apikey_desc'] = 'Необязательно. Большинство самостоятельно размещённых серверов не требуют ключа в доверенной сети; устанавливайте это только если ваш сервер требует bearer-токен.';
$string['emergency:title'] = 'Экстренное управление [[tutorshort]]';
$string['emergency:page_warning'] = 'Эти переключатели вступают в силу немедленно для каждого учащегося на сайте. Каждое действие записывает строку аудита. Гранулярные переключатели оставляют остальную часть [[tutorshort]] работающей; главное отключение полностью удаляет виджет.';
$string['emergency:back_to_settings'] = 'Настройки [[tutorshort]]';
$string['emergency:state_disabled'] = 'ОТКЛЮЧЕНО';
$string['emergency:state_active'] = 'Активно';
$string['emergency:confirm_label'] = 'Я понимаю, что это немедленно затронет каждого учащегося';
$string['emergency:confirm_required'] = 'Установите флажок подтверждения перед отключением подсистемы.';
$string['emergency:reason_placeholder'] = 'Причина (записывается в журнал аудита)';
$string['emergency:disable_button'] = 'Отключить';
$string['emergency:restore_button'] = 'Восстановить';
$string['emergency:disabled_notice'] = 'Подсистема «{$a->flag}» отключена. Затронутая конфигурация: {$a->touched}';
$string['emergency:restored_notice'] = 'Подсистема «{$a->flag}» восстановлена. Затронутая конфигурация: {$a->touched}';
$string['emergency:cli_reference'] = 'Те же элементы управления доступны из дежурной оболочки:';
$string['emergency:flag_chat'] = 'Чат';
$string['emergency:flag_chat_desc'] = 'Блокирует трафик чата через специальный флаг отключения (исправление v5.13). Виджет продолжает отображаться; учащиеся видят дружелюбное сообщение «[[tutorshort]] приостановлена». Используйте, когда провайдер LLM ведёт себя неправильно или происходит скачок затрат.';
$string['emergency:flag_voice'] = 'Голос';
$string['emergency:flag_voice_desc'] = 'Очищает активного провайдера голоса в реальном времени (сохранённого для точного восстановления). Текстовый чат продолжает работать.';
$string['emergency:flag_rag'] = 'RAG';
$string['emergency:flag_rag_desc'] = 'Отключает поиск и индексирование. Чат продолжает работать без привязки к содержимому курса.';
$string['emergency:flag_outreach'] = 'Рассылка';
$string['emergency:flag_outreach_desc'] = 'Останавливает дайджест, этапные и напоминательные письма. Чат не затронут.';
$string['emergency:flag_all'] = 'ГЛАВНОЕ ОТКЛЮЧЕНИЕ';
$string['emergency:flag_all_desc'] = 'Отключает весь плагин: виджет исчезает с каждой страницы, запланированные задачи останавливаются, голос очищен, RAG отключён, рассылка отключена. Самый мощный переключатель — используйте при инциденте безопасности или когда [[tutorshort]] необходимо немедленно перевести в офлайн.';
$string['emergency:settings_link'] = 'Экстренное управление';
$string['emergency:settings_link_desc'] = 'Переключатели отключения по подсистемам (чат / голос / RAG / рассылка / главный) с ведением журнала аудита — веб-эквивалент <code>admin/cli/emergency_disable.php</code>. Откройте <a href="{$a}">Экстренное управление [[tutorshort]]</a>.';
$string['email_unsubscribe:done_title'] = 'Отписка выполнена';
$string['email_unsubscribe:done_body'] = 'Готово — {$a->email} больше не будет получать этот тип писем от {$a->product}. Если вы передумаете, попросите администратора {$a->product} повторно включить подписку или отправьте новый запрос на подписку через страницу [[tutorshort]] Recipients в панели администратора.';
$string['email_unsubscribe:invalid_title'] = 'Ссылка для отписки больше не действительна';
$string['email_unsubscribe:invalid_body'] = 'Эта ссылка для отписки истекла или некорректна. Поищите более позднее письмо от нас или свяжитесь с администратором сайта для ручного удаления.';
$string['settings:prompt_proportions_heading'] = 'Пропорции разделов подсказки (v5.6.0)';
$string['settings:prompt_proportions_heading_desc'] = 'Распределите бюджет системной подсказки по четырём сегментам: безопасность + идентичность, структура курса, содержимое курса и текущая страница. Веса задаются в процентах с суммой 100. Эмпирически настроенные значения по умолчанию (10 / 10 / 40 / 40) взяты из тестирования настройки весов v5.6.0; оставление текстовой области пустой применяет эти значения по умолчанию. Автоматическое усиление корректирует распределение per-ход в зависимости от того, находится ли конкретная страница в зоне охвата.';
$string['settings:prompt_section_weights'] = 'Базовые веса разделов (JSON)';
$string['settings:prompt_section_weights_desc'] = 'Необязательный объект JSON, отображающий каждый сегмент на процентное значение. Оставьте пустым для использования значений из теста (10 / 10 / 40 / 40). Пример: <code>{"safety_identity": 10, "course_structure": 10, "course_content": 40, "current_page": 40}</code>. Веса должны суммироваться до 100 (±5%). Для <code>safety_identity</code> установлен минимальный порог 10%, чтобы защита от джейлбрейка и маркеры формата вывода всегда были включены полностью. <code>current_page + course_content</code> должны быть не менее 40%, чтобы у модели был существенный материал для опоры. Значения вне диапазона молча возвращаются к значениям теста; администраторы должны проверять, просматривая журнал отладки подсказок после сохранения.';
$string['settings:prompt_context_boost_mode'] = 'Режим усиления контекста';
$string['settings:prompt_context_boost_mode_desc'] = 'Автоматическая настройка, которая смещает вес в сторону раздела текущей страницы, когда конкретная страница находится в зоне охвата, и в сторону содержимого курса, когда страница не выбрана. <strong>page_focus</strong> (по умолчанию) смещает около 15 весовых пунктов. <strong>aggressive</strong> смещает 25 пунктов и лучше всего подходит, когда учащиеся систематически задают вопросы, связанные с конкретными страницами. <strong>off</strong> отключает усиление; установленные администратором базовые веса применяются в каждом ходу независимо от контекста страницы.';
$string['settings:prompt_context_boost_off'] = 'Выключено (применять базовые веса в каждом ходу)';
$string['settings:prompt_context_boost_page_focus'] = 'Фокус на странице (по умолчанию, ~15 пунктов смещения)';
$string['settings:prompt_context_boost_aggressive'] = 'Агрессивный (~25 пунктов смещения)';
$string['settings:prompt_section_weights_coach'] = 'Переопределение в режиме тренера (JSON, необязательно)';
$string['settings:prompt_section_weights_coach_desc'] = 'Необязательный объект JSON, переопределяющий базовые веса разделов специально во время режима тренера в оцениваемой викторине (когда <code>quizmode=coach</code>). Полезен для принудительного назначения большего веса <code>current_page</code> во время викторин без влияния на обычный чат. Оставьте пустым для наследования базовых весов. Те же правила проверки, что и для базовой настройки.';
$string['prompt_debug_view:title'] = 'Просмотрщик журнала отладки подсказок';
$string['prompt_debug_view:subtitle'] = 'Системная подсказка, собранная per-ход + разбивка по разделам + история разговора + текущее сообщение пользователя + метаданные вложения, именно так, как их получила модель. Используйте это для проверки того, действительно ли раздел «Содержимое текущей страницы» попал в подсказку, и для отладки проблем с качеством ответов без SSH-подключения к серверу.';
$string['prompt_debug_view:disabled'] = 'Ведение журнала отладки подсказок в данный момент ВЫКЛЮЧЕНО. Новые записи не будут создаваться до его включения.';
$string['prompt_debug_view:enable_link'] = 'Откройте настройки плагина, чтобы включить «Записывать собранную системную подсказку в файл».';
$string['prompt_debug_view:no_log_yet'] = 'Файл журнала пока отсутствует. Отправьте хотя бы один ход чата после включения журнала отладки; файл создаётся при первой записи.';
$string['prompt_debug_view:empty'] = 'Файл журнала существует, но пуст. Отправьте ход чата и обновите страницу.';
$string['prompt_debug_view:file_status'] = 'Размер файла журнала';
$string['prompt_debug_view:showing'] = 'Отображаются последние записи (сначала новые), лимит';
$string['prompt_debug_view:total'] = 'Всего подсказка';
$string['prompt_debug_view:budget'] = 'Бюджет при захвате';
$string['prompt_debug_view:sections'] = 'Разделы (по категориям)';
$string['prompt_debug_view:assembled_prompt'] = 'Собранная системная подсказка';
$string['prompt_debug_view:history'] = 'История разговора, отправленная модели';
$string['prompt_debug_view:current_message'] = 'Текущее сообщение пользователя';
$string['prompt_debug_view:attachment'] = 'Метаданные вложения';
$string['prompt_debug_view:show_more'] = 'Показать больше записей';
$string['settings:mastery_classifier_provider'] = 'Провайдер классификатора';
$string['settings:mastery_classifier_provider_desc'] = 'Идентификатор провайдера, используемый для per-ходового классификатора освоения. Оставьте пустым для наследования провайдера AI по умолчанию. Стандартный <code>openai</code> в паре с моделью классификатора <code>gpt-4o-mini</code> ниже — самый дешёвый вариант TIER 1 для классификации со структурированным выводом (~$220/мес. экономии при 100k MAU по сравнению с уровнем чата). При настройке строка в Провайдерах сравнения с этим идентификатором провайдера предоставляет ключ API, базовый URL и температуру.';
$string['settings:premium_escalation_heading'] = 'Уровень премиум-эскалации (A.10)';
$string['settings:premium_escalation_heading_desc'] = 'Необязательная per-ходовая маршрутизация к премиум-модели (по умолчанию Claude Opus 4.8) для подсказок, с которыми рабочий уровень чата явно не справляется — обычно многошаговая математика, CS и научное мышление. Разрешено тестом A.10 от 2026-06-09: Opus 4.8 победил с результатом 14,97/15 против 12,68/15 у gpt-4o на сложных подсказках. Два пути срабатывания: совпадения regex в сообщении пользователя ИЛИ список разрешённых курсов, эскалирующий каждый ход. По умолчанию выключено. При ~5% эскалации ожидайте ~$700/месяц при 100k [[unishort]] MAU сверх базовых расходов на чат.';
$string['settings:premium_escalation_enabled'] = 'Включить маршрутизацию премиум-эскалации';
$string['settings:premium_escalation_enabled_desc'] = 'При включении per-ходовой маршрутизатор проверяет список regex-триггеров и список разрешённых курсов для каждого вызова чата; совпадающие ходы маршрутизируются к премиум-провайдеру. Возвращается к рабочему провайдеру, если строка премиума отсутствует или не удаётся создать экземпляр. Переопределения выбора LLM администратора всегда имеют приоритет.';
$string['settings:premium_escalation_provider'] = 'Премиум-провайдер';
$string['settings:premium_escalation_provider_desc'] = 'Идентификатор провайдера для маршрутизации премиум-вызовов. Должен соответствовать строке в Провайдерах сравнения (чтобы ключ API, базовый URL и температура поступали из того же места, которым уже управляют администраторы). По умолчанию <code>claude</code>.';
$string['settings:premium_escalation_model'] = 'Премиум-модель';
$string['settings:premium_escalation_model_desc'] = 'Имя модели, передаваемое премиум-провайдеру. По умолчанию <code>claude-opus-4-8</code> согласно вердикту теста A.10.';
$string['settings:premium_escalation_triggers'] = 'Regex-триггеры премиума';
$string['settings:premium_escalation_triggers_desc'] = 'Одно выражение PCRE regex на строку (без разделителей; совпадение без учёта регистра применяется автоматически). Строки, начинающиеся с #, являются комментариями. Оставьте пустым для использования тщательно отобранного набора по умолчанию из теста A.10 (маркеры многошагового STEM: «derive», «prove that», «step by step», математика LaTeX, огороженные блоки кода, big-O, интегралы, оптимизация и т.д.).';
$string['settings:premium_escalation_course_tags'] = 'Список разрешённых курсов для премиума';
$string['settings:premium_escalation_course_tags_desc'] = 'Один префикс краткого названия курса или idnumber на строку. Каждый ход в совпадающем курсе автоматически эскалируется независимо от regex сообщения (используйте для STEM-интенсивных курсов, где эскалация должна быть по умолчанию). Совпадение — без учёта регистра по префиксу — «MATH» совпадает с MATH121, MATH205 и т.д.';
$string['settings:spend_cap_per_course_default'] = 'Стандартный лимит расходов на курс (USD)';
$string['settings:spend_cap_per_course_default_desc'] = 'Защитный лимит, применяемый к каждому курсу, у которого нет настроенного собственного лимита расходов на курс. Установите, например, <code>30</code>, чтобы ограничить месячные расходы любого курса на уровне $30 без настройки отдельных курсов. <code>0</code> = без стандартного лимита (применяются только общесайтовые лимиты и переопределения по курсу). Когда курс превысит 80% / 95% / 100% этого лимита, существующий конвейер оповещений spend-guard отправляет уведомление администратору (список получателей: <code>spend_notify_emails</code>, с резервным вариантом для администраторов сайта). Конкретный курс всегда может поднять собственный потолок, установив более высокое переопределение по курсу.';
$string['settings:cost_anomaly_heading'] = 'Детектор аномалий затрат (v6.0)';
$string['settings:cost_anomaly_heading_desc'] = 'Ежедневная запланированная задача (<code>cost_anomaly_check</code>), сравнивающая сегодняшние общесайтовые расходы [[tutorshort]] со скользящей медианой за 7 дней. Отправляет электронное письмо списку получателей <code>spend_notify_emails</code> (с резервным вариантом для администраторов сайта), когда сегодняшние расходы превышают настроенный множитель × медиана. Обнаруживает три режима сбоя, которые пропускают существующие пороговые значения 80% / 95% / 100% лимита расходов: (1) неконтролируемый курс, где абсолютный потолок не превышен, но один курс внезапно генерирует 10x обычного объёма, (2) случайное включение премиум-уровня, (3) неправильная маршрутизация провайдера. По умолчанию выключено; внутренний эквивалент [[tutorshort]] запроса Redash по адресу <code>.drafts/sola-redash-cost-anomaly-2026-06-09.md</code>.';
$string['settings:cost_anomaly_enabled'] = 'Включить детектор аномалий затрат';
$string['settings:cost_anomaly_enabled_desc'] = 'При включении ежедневная запланированная задача оценивает сегодняшние расходы по сравнению со скользящей медианой за 7 дней и отправляет письмо администраторам при аномалии. Первые 7 дней после включения дают статус <code>insufficient_history</code> (исторической базы ещё нет) и никогда не генерируют оповещение. Идемпотентно по дням: флаг в <code>config_plugins</code> останавливает повторные письма, если cron запускается несколько раз.';
$string['settings:cost_anomaly_multiplier'] = 'Множитель аномалии';
$string['settings:cost_anomaly_multiplier_desc'] = 'Сегодняшние расходы должны превысить этот множитель × медиану за 7 дней для срабатывания оповещения. По умолчанию <code>2.0</code>. Уменьшите до <code>1.5</code> для более ранних предупреждений (больше ложных срабатываний в периоды всплеска записи). Увеличьте до <code>3.0</code>, если использование [[unishort]] достаточно неравномерное, чтобы скачки в 2x были рутиной.';
$string['task:cost_anomaly_check'] = 'Ежедневная проверка аномалий затрат [[tutorshort]]';

// v6.4.0 signed policy bundle strings (added 2026-06-11).
$string['settings:policy_bundle_heading'] = 'Подписанный пакет политик (удалённые обновления поведения)';
$string['settings:policy_bundle_heading_desc'] = 'Применяет настройки поведения (подсказки, маршрутизацию, триггеры эскалации, настройку RAG, политику расходов) из криптографически подписанного файла JSON без развёртывания кода. Ежедневная запланированная задача получает URL пакета, проверяет его подпись Ed25519 по публичному ключу ниже и применяет настройки только если каждый ключ присутствует во встроенном списке разрешённых и версия пакета новее последней применённой. Ключи API, URL-адреса, вебхуки и настройки безопасности не могут быть установлены пакетом. Создавайте и подписывайте пакеты с помощью <code>admin/cli/policy_bundle_tool.php</code> (keygen, sign, verify, status, sync).';
$string['settings:policy_bundle_enabled'] = 'Включить синхронизацию пакета политик';
$string['settings:policy_bundle_enabled_desc'] = 'При включении ежедневная задача получает и применяет подписанные пакеты. По умолчанию отключено. Отключение немедленно останавливает все синхронизации; уже применённые настройки сохраняют свои значения.';
$string['settings:policy_bundle_url'] = 'URL пакета политик';
$string['settings:policy_bundle_url_desc'] = 'URL HTTPS подписанного JSON пакета (например, объект S3 или GitHub raw URL). URL проходит ту же проверку SSRF, что и конечные точки провайдеров AI; хосты в частной сети или plain-http требуют записи в списке доверенных конечных точек SSRF.';
$string['settings:policy_bundle_pubkey'] = 'Публичный ключ пакета политик';
$string['settings:policy_bundle_pubkey_desc'] = 'Публичный ключ Base64 Ed25519 для проверки подписей пакета. Сгенерируйте пару ключей с помощью <code>policy_bundle_tool.php --keygen</code>; закрытый ключ остаётся у автора пакета и никогда не должен никуда загружаться.';
$string['settings:policy_bundle_status'] = 'Последняя синхронизация';
$string['settings:policy_bundle_applied_version'] = 'применённая версия';
$string['task:policy_bundle_sync'] = 'Синхронизация подписанного пакета политик [[tutorshort]]';
$string['policy_bundle:invalid'] = 'Пакет политик отклонён: {$a}';
$string['prompt_debug_view:retrieved_chunks'] = 'Извлеченные фрагменты (выбор RAG)';
$string['prompt_debug_view:retrieved_chunks_hint'] = 'Фрагменты, которые система извлечения выбрала для этого вопроса, в порядке ранжирования с их оценкой релевантности и источником (cmid). Используйте это, чтобы убедиться, что модель получила наиболее подходящий учебный материал.';
$string['settings:avatar_animation_enabled'] = 'Анимация аватара';
$string['settings:avatar_animation_enabled_desc'] = 'Анимируйте созданный SVG-аватар: мигание в режиме ожидания, а также движение рта, синхронизированное со звуком преобразования текста в речь, пока говорит ассистент. Учитывает предпочтение сниженного движения устройства учащегося. Переопределение для каждого курса для измерения A/B: установите значение конфигурации avatar_animation_course_COURSEID равным 0 или 1.';
$string['analytics:exp_heading'] = 'Сравнение эксперимента A/B';
$string['analytics:exp_desc'] = 'Сравните вовлечённость между двумя курсами за выбранный период. Создан для экспериментов по курсам (например, зонд анимации аватара): поместите переопределение в один курс, оставьте другой в качестве контроля и прочитайте разницу здесь.';
$string['analytics:exp_course_a'] = 'Курс A';
$string['analytics:exp_course_b'] = 'Курс B';
$string['analytics:exp_compare'] = 'Сравнить';
$string['analytics:exp_metric'] = 'Метрика';
$string['analytics:exp_delta'] = 'B vs A';
$string['analytics:exp_enrolled'] = 'Зарегистрированные учащиеся';
$string['analytics:exp_active_users'] = 'Активные пользователи [[tutorshort]]';
$string['analytics:exp_usage_rate'] = 'Процент использования (%)';
$string['analytics:exp_sessions'] = 'Сессии';
$string['analytics:exp_messages'] = 'Сообщения';
$string['analytics:exp_avg_msgs_session'] = 'Среднее количество сообщений на сессию';
$string['analytics:exp_avg_session_minutes'] = 'Средняя продолжительность сессии (минуты)';
$string['analytics:exp_return_rate'] = 'Возвращающиеся пользователи (%)';
$string['analytics:exp_tts_plays'] = 'Воспроизведения TTS';
$string['analytics:exp_tts_per_active'] = 'Воспроизведений TTS на активного пользователя';

$string['settings:redash_allowed_origin'] = 'Разрешённый источник для Redash (CORS)';
$string['settings:redash_allowed_origin_desc'] = 'Оставьте пустым (рекомендуется): экспорт извлекается Redash с сервера на сервер и не требует браузерного заголовка CORS. Указывайте единственный точный источник (например https://redash.example.org) только в том случае, если браузерная панель мониторинга должна читать экспорт напрямую. Никогда не используйте подстановочный знак.';

// Soapbox speech practice (v6.7.0).
$string['privacy:metadata:local_ai_course_assistant_practice_scores:session_meta'] = 'Необязательные метаданные, которые вы указали для сеанса, например название, тема и целевая длительность выступления Soapbox. Никогда не включает аудио или расшифровку.';
$string['pedagogy:soapbox'] = 'Отзывы о выступлениях Soapbox включены по умолчанию';
$string['pedagogy:soapbox_desc'] = 'Когда включено, инструмент практики выступлений Soapbox доступен в каждом курсе, если у курса нет собственного переопределения. Оставьте выключенным и включайте его только в тех курсах, где он нужен (как правило, курсы ораторского мастерства и коммуникации).';
$string['settings:soapbox_stt_mode'] = 'Режим расшифровки Soapbox';
$string['settings:soapbox_stt_mode_desc'] = 'Как Soapbox превращает записанное выступление в текст. Сервер использует настроенного провайдера Whisper (самостоятельный хостинг бесплатен; размещённый OpenAI стоит около USD 0,006 за минуту). Браузер использует встроенное распознавание речи учащегося (бесплатно, без сервера, работает только в Chrome и Safari). Рекомендуется режим «Сервер», чтобы качество расшифровки не зависело от браузера учащегося.';
$string['settings:soapbox_stt_mode_server'] = 'Сервер (провайдер Whisper)';
$string['settings:soapbox_stt_mode_browser'] = 'Браузер (бесплатно, без сервера)';
$string['soapbox:title'] = 'Soapbox';
$string['soapbox:link'] = 'Практика выступлений Soapbox';
$string['soapbox:disabled'] = 'Soapbox не включён для этого курса.';
$string['soapbox:intro'] = 'Произнесите речь и получите наставничество. При желании укажите название, тему и целевую длительность, затем запишите своё выступление. Soapbox расшифрует вашу речь, оценит её по критериям ораторского мастерства и даст конкретные советы. Ваше аудио и расшифровка никогда не сохраняются — только ваши оценки и отзывы.';
$string['soapbox:optional'] = 'необязательно';
$string['soapbox:name_label'] = 'Назовите это выступление';
$string['soapbox:topic_label'] = 'Тема';
$string['soapbox:time_label'] = 'Целевая длительность';
$string['soapbox:no_target'] = 'Без цели';
$string['soapbox:record'] = 'Записать выступление';
$string['soapbox:stop'] = 'Остановить';
$string['soapbox:recording'] = 'Идёт запись. Говорите естественно; нажмите «Остановить», когда закончите.';
$string['soapbox:transcribing'] = 'Расшифровка вашего выступления…';
$string['soapbox:scoring'] = 'Оценка вашего выступления…';
$string['soapbox:too_short'] = 'Эта запись слишком короткая для оценки. Постарайтесь произнести хотя бы пару предложений и попробуйте снова.';
$string['soapbox:mic_denied'] = 'Для записи нужен доступ к микрофону. Разрешите доступ к микрофону и попробуйте снова.';
$string['soapbox:no_browser_stt'] = 'Этот браузер не поддерживает распознавание речи в браузере. Попробуйте Chrome или Safari или попросите администратора переключить Soapbox на серверную расшифровку.';
$string['soapbox:browser_note'] = 'Это выступление расшифровывается в вашем браузере. Ничего не загружается на сервер. Лучше всего работает в Chrome и Safari.';
$string['soapbox:server_note'] = 'Ваша запись загружается только для расшифровки и не сохраняется.';
$string['soapbox:error'] = 'Сейчас не удалось оценить это выступление. Попробуйте снова через мгновение.';
$string['soapbox:audio_too_large'] = 'Эта запись слишком большая. Старайтесь, чтобы выступления были меньше примерно 25 MB (около 20 минут).';
$string['soapbox:no_stt'] = 'Провайдер расшифровки не настроен. Попросите администратора настроить Whisper или включить расшифровку в браузере.';
$string['soapbox:result_heading'] = 'Оценки по критериям';
$string['soapbox:overall_heading'] = 'Итого';
$string['soapbox:tips_heading'] = 'Советы на следующий раз';
$string['soapbox:col_criterion'] = 'Критерий';
$string['soapbox:col_score'] = 'Оценка';
$string['soapbox:col_feedback'] = 'Отзыв';
$string['soapbox:history_heading'] = 'Мои выступления';
$string['soapbox:history_empty'] = 'Вы ещё не записали ни одного выступления. Запишите его выше, чтобы начать формировать свою историю.';
$string['soapbox:untitled'] = 'Выступление без названия';
$string['soapbox:overall_badge'] = 'Итого {$a}';
$string['soapbox:toggle'] = 'Включить Soapbox для этого курса';
$string['soapbox:toggle_help'] = 'Учащиеся получают отдельную страницу для записи выступления и получения отзывов об ораторском мастерстве с оценкой по критериям и советами. Аудио и расшифровки никогда не сохраняются. По умолчанию выключено.';
// Soapbox course-type/level + sample loader (v6.7.0).
$string['soapbox:level_label'] = 'Тип курса / уровень выступления';
$string['soapbox:level_help'] = 'Настраивает коучинг ИИ и образец рубрики по умолчанию под тип курса. Уровни ESL получают обратную связь по изучению языка; общее выступление сосредоточено на навыках презентации. Рубрику ниже можно по-прежнему редактировать.';
$string['soapbox:level_general'] = 'Общее выступление / презентация';
$string['soapbox:level_esl_beginner'] = 'ESL (начальный уровень)';
$string['soapbox:level_esl_advanced'] = 'ESL (продвинутый уровень)';
$string['soapbox:edit_rubric'] = 'Редактировать рубрику выступления';
$string['soapbox:sample_label'] = 'Загрузить образец рубрики';
$string['soapbox:sample_choose'] = 'Выберите образец…';
$string['soapbox:sample_hint'] = 'Загружает примерные критерии в редактор ниже. Просмотрите и сохраните, чтобы применить их к этой области.';
$string['soapbox:level_esl_intermediate'] = 'ESL (средний уровень)';
...
$string['cachedef_ratelimit'] = 'Ограничение частоты запросов на пользователя';
$string['cachedef_uistate'] = 'Переключатели интерфейса на сессию (просмотр как студент, показ настоящих имён)';
$string['benchmark:pagetitle'] = 'Тест производительности провайдеров';
$string['benchmark:intro'] = 'Отправляет фиксированный набор типичных запросов каждому настроенному AI-провайдеру, фиксирует расход токенов, стоимость и задержку и рекомендует по одному провайдеру на каждую возможность. Каждый запуск выполняет реальные вызовы API, поэтому один запуск стоит примерно от 5 до 20 центов в зависимости от количества настроенных провайдеров.';
$string['cachedef_systemprompt'] = 'Собранный системный запрос AI (для каждого курса)';
$string['cachedef_remoteconfig'] = 'Удалённая конфигурация, полученная из канала обновлений';
$string['cachedef_spend'] = 'Итоги расходов на AI за период для контроля расходов';
$string['cachedef_failover_circuit'] = 'Состояние предохранителя переключения между провайдерами';
$string['settings:rag_scope'] = 'Область поиска';
$string['settings:rag_scope_desc'] = 'Когда учащийся просматривает конкретный документ (страницу, книгу или PDF), нужно ли ограничивать поиск этим документом. <b>Сначала документ</b> основывает ответ на текущем документе, если в нём есть подходящие фрагменты, и иначе возвращается ко всему курсу. <b>Только документ</b> никогда не возвращается назад: если в текущем документе нет подходящего фрагмента, ничего не извлекается, поэтому тьютор отвечает на основе общих знаний, а не ссылается на несвязанные страницы. <b>По всему курсу</b> ищет по всему содержимому курса (текущая страница всё равно получает приоритет в порядке, указанном выше). Рекомендуется режим «Сначала документ», чтобы ответ о странице, которую читает учащийся, был основан на этой странице.';
$string['settings:rag_scope_document_first'] = 'Сначала документ (предпочесть текущий документ, вернуться к курсу)';
$string['settings:rag_scope_document_only'] = 'Только документ (ограничить текущим документом)';
$string['settings:rag_scope_course'] = 'По всему курсу (искать по всему содержимому курса)';
$string['ragadmin:no_content_documents'] = '{$a->count} документ(ов) не дали индексируемого текста и не были разбиты на фрагменты (обычно это преимущественно встроенный контент или очень короткие страницы): {$a->titles}. Если какие-либо из них должны быть доступны для поиска, добавьте текст на страницу или расшифровку, затем переиндексируйте.';
$string['privacy:metadata:sbx_rec'] = 'Записи презентаций Soapbox. Медиаобъект удаляется после срока хранения; расшифровка сохраняется для обратной связи и запросов на доступ.';
$string['privacy:metadata:sbx_rec:userid'] = 'Учащийся, сделавший запись.';
$string['privacy:metadata:sbx_rec:transcript'] = 'Расшифровка записи, преобразованная из речи в текст.';
$string['privacy:metadata:sbx_rec:duration_seconds'] = 'Длительность записи в секундах.';
$string['privacy:metadata:sbx_rec:timecreated'] = 'Когда была сделана запись.';
$string['privacy:metadata:ai_provider'] = 'Для генерации ответов тьютора созданный учащимся контент отправляется AI-провайдеру, настроенному администратором сайта или курса.';
$string['privacy:metadata:ai_provider:message'] = 'Текст сообщения и вложения, которые учащийся отправляет AI-тьютору.';
$string['privacy:metadata:ai_provider:coursecontext'] = 'Контекст курса и деятельности (например, название курса и текущая страница), включённый для обоснования ответа.';
$string['privacy:metadata:voice_provider'] = 'Когда используется голосовой ввод или вывод, аудио и текст отправляются настроенному провайдеру речи в текст / текста в речь.';
$string['privacy:metadata:voice_provider:audio'] = 'Записанное аудио речи учащегося, отправленное для расшифровки.';
$string['privacy:metadata:voice_provider:text'] = 'Текст, отправленный провайдеру для синтеза в речь.';
$string['privacy:metadata:zendesk'] = 'Когда включена передача обращений в поддержку и учащийся даёт согласие, его контактные данные и сообщение отправляются в Zendesk для создания обращения в поддержку.';
$string['privacy:metadata:zendesk:name'] = 'Полное имя учащегося, используемое для идентификации автора обращения в поддержку.';
$string['privacy:metadata:zendesk:email'] = 'Адрес электронной почты учащегося, используемый для ответа на обращение в поддержку.';
$string['privacy:metadata:zendesk:message'] = 'Содержимое сообщения или беседы, которое учащийся решил передать в поддержку.';
$string['privacy:metadata:radar_webhook'] = 'Когда настроена доставка Learning Radar на веб-хук Slack или Microsoft Teams, сгенерированный отчёт публикуется на этой внешней конечной точке.';
$string['privacy:metadata:radar_webhook:report'] = 'Содержимое отчёта Learning Radar, которое может ссылаться на активность учащегося в курсе.';
$string['instructor_dashboard:navlink'] = 'Панель AI-тьютора';
$string['analytics_js:total_students'] = 'Всего студентов';
$string['analytics_js:active_ai_users'] = 'Активные пользователи AI';
$string['analytics_js:msgs_per_student'] = 'Сообщений / студент';
$string['analytics_js:avg_session'] = 'Средняя сессия';
$string['analytics_js:return_rate'] = 'Доля возвратов';
$string['analytics_js:total_sessions'] = 'Всего сессий';
$string['analytics_js:ai_users'] = 'Пользователи AI';
$string['analytics_js:non_users'] = 'Не пользователи';
$string['analytics_js:thumbs_up'] = 'Лайки';
$string['analytics_js:thumbs_down'] = 'Дизлайки';
$string['analytics_js:hallucination_flags'] = 'Отметки о галлюцинациях';
$string['analytics_js:avg_star_rating'] = 'Средняя оценка в звёздах';
$string['analytics_js:avg_msgs_resolution'] = 'Среднее число сообщений до решения';
$string['analytics_js:survey_respondents'] = 'Респонденты опроса';
$string['analytics_js:messages'] = 'Сообщения';
$string['analytics_js:students'] = 'Студенты';
$string['analytics_js:frequency'] = 'Частота';
$string['analytics_js:responses'] = 'Ответы';
$string['analytics_js:error_loading'] = 'Ошибка загрузки данных';
$string['analytics_js:loading'] = 'Загрузка аналитики...';
$string['analytics_js:no_course_data'] = 'Нет доступных данных по курсу.';
$string['analytics_js:no_unit_data'] = 'Пока нет данных по разделам. Отслеживание разделов накапливается по мере использования помощника студентами.';
$string['analytics_js:no_keyword_data'] = 'Нет данных по ключевым словам за этот период.';
$string['task:soapbox_cleanup'] = 'Хранение и очистка записей Soapbox';
$string['admin:vendor_dpa:col_provider'] = 'Провайдер';
$string['admin:vendor_dpa:col_optout'] = 'Отказ от обучения';
$string['admin:vendor_dpa:col_dpa'] = 'DPA';
$string['admin:vendor_dpa:col_retention'] = 'Срок хранения';
$string['admin:vendor_dpa:col_tier'] = 'Максимальный уровень';
$string['admin:vendor_dpa:col_link'] = 'Ссылка';
$string['admin:vendor_dpa:vendor_terms'] = 'Условия поставщика';
$string['admin:vendor_dpa:tier'] = 'Уровень {$a}';
$string['admin:vendor_dpa:too_contractual'] = 'По договору (отказ оформлен)';
$string['admin:vendor_dpa:too_default_on'] = 'Включено по умолчанию';
$string['admin:vendor_dpa:too_none'] = 'Без отказа';
$string['admin:vendor_dpa:too_local'] = 'Локально (без поставщика)';
$string['admin:vendor_dpa:too_unknown'] = 'Ещё не проверено';
$string['admin:vendor_dpa:dpa_signed'] = 'Подписано';
$string['admin:vendor_dpa:dpa_available'] = 'Доступно';
$string['admin:vendor_dpa:dpa_negotiating'] = 'На согласовании';
$string['admin:vendor_dpa:dpa_not_offered'] = 'Не предлагается';
$string['admin:vendor_dpa:dpa_not_applicable'] = 'Н/П';
$string['admin:vendor_dpa:dpa_unknown'] = 'Неизвестно';
$string['settings:soapbox_slide_vision'] = 'Обратная связь по визуальному оформлению слайдов Soapbox';
$string['settings:soapbox_slide_vision_desc'] = 'Разрешить один визуальный проход по отрисованным изображениям слайдов, чтобы добавить краткую заметку о визуальном оформлении к оценённой презентации. Отключено по умолчанию и учитывает конфиденциальность: изображения не сохраняются, и каждое задание также должно быть включено собственным флажком «Обратная связь по визуальному оформлению слайдов». Затрагиваются только презентации со слайдами, к которым загружена колода.';
$string['settings:soapbox_vision_provider'] = 'Провайдер анализа слайдов Soapbox';
$string['settings:soapbox_vision_provider_desc'] = 'Провайдер, используемый для прохода анализа слайдов. По умолчанию openai. Должен быть провайдером с поддержкой обработки изображений, настроенным с ключом API (через провайдера курса или список провайдеров для сравнения).';
$string['settings:soapbox_vision_model'] = 'Модель анализа слайдов Soapbox';
$string['settings:soapbox_vision_model_desc'] = 'Модель с поддержкой обработки изображений для прохода анализа слайдов. По умолчанию gpt-4o-mini.';
$string['soapbox:slide_design_note'] = 'Оформление слайдов:';
$string['soapbox:assign_manage'] = 'Задания на презентацию Soapbox';
$string['soapbox:copy_link'] = 'Ссылка для студента';
$string['soapbox:deck_label'] = 'Слайды (PDF). Загрузите свою колоду, затем переключайте слайды во время записи.';
$string['soapbox:slide_prev'] = 'Предыдущий слайд';
$string['soapbox:slide_next'] = 'Следующий слайд';
$string['soapbox:play_slides'] = 'Воспроизвести со слайдами';
$string['soapbox:audio_ready'] = 'Только аудио. Ваш микрофон будет записан; камера не используется.';
$string['soapbox:audio_recording'] = 'Запись аудио...';
$string['soapbox:cap_reached'] = 'Вы достигли максимального количества записей для этого задания.';
$string['soapbox:storage_unconfigured'] = 'Хранилище записей ещё не настроено. Пожалуйста, обратитесь к вашему администратору.';
$string['soapbox:bad_key'] = 'Эта загрузка вам не принадлежит.';
$string['soapbox:slides_disabled'] = 'Слайды не включены для этого задания.';
$string['soapbox:upload_missing'] = 'Не удалось найти загрузку. Пожалуйста, попробуйте записать снова.';
$string['soapbox:mode_label'] = 'Тип презентации';
$string['soapbox:mode_informative'] = 'Информативная (объяснить или обучить)';
$string['soapbox:mode_persuasive'] = 'Убеждающая (убедить)';
$string['soapbox:err_provider'] = 'Не удалось связаться с сервисом оценки AI. Пожалуйста, попробуйте снова через мгновение.';
$string['soapbox:err_parse'] = 'AI вернул неожиданный ответ. Пожалуйста, попробуйте снова.';
$string['soapbox:err_disabled'] = 'Практика речи не включена для этого курса.';
$string['soapbox:err_transcribe'] = 'Не удалось расшифровать вашу запись. Пожалуйста, попробуйте снова и проверьте, работает ли ваш микрофон.';
$string['settings:outcomes_benchmark_default'] = 'Целевой показатель результата (процент)';
$string['settings:outcomes_benchmark_default_desc'] = 'Установленный учреждением целевой показатель для отчёта о результатах обучения: студент достигает результата, когда его уровень освоения равен этому проценту или выше. Это стандарт отчётности (отличается от порога освоения для коучинга). По умолчанию 70.';
$string['outcomes:title'] = 'Отчёт о результатах обучения';
$string['outcomes:intro'] = 'Для каждого результата обучения курса указаны целевой показатель и процент оценённых студентов, которые достигли его или превысили. Целевой показатель сайта — {$a}%.';
$string['outcomes:none'] = 'Для этого курса пока не определены результаты обучения.';
$string['outcomes:export'] = 'Экспорт в CSV';
$string['outcomes:col_outcome'] = 'Результат обучения';
$string['outcomes:col_benchmark'] = 'Целевой показатель';
$string['outcomes:col_assessed'] = 'Оценено студентов';
$string['outcomes:col_met'] = 'Достигли показателя';
$string['outcomes:col_pct'] = 'Процент достигших';
$string['outcomes:footnote'] = 'Оценёнными считаются студенты, сделавшие хотя бы одну попытку по результату обучения. Достижение отражается в совокупности; индивидуальное продвижение никогда не ограничивается ни одним отдельным результатом.';
$string['outcomes:navlink'] = 'Отчёт о результатах обучения';

// v6.9.5 i18n batch: RAG budget cap, Redash export window and de-anonymisation gate.
$string['chat:refused'] = 'Я не могу помочь с этим запросом. Вернёмся к вашему курсу: над чем хотите поработать сейчас?';
$string['settings:rerank_margin_threshold'] = 'Порог неоднозначности переранжирования';
$string['settings:rerank_margin_threshold_desc'] = 'Переранжировать только тогда, когда косинусная разница между первым и третьим кандидатом ниже этого значения, то есть когда поиск неоднозначен. Измерено на 1008 запросах: при значении по умолчанию 0,086 пропускается около 30 % запросов без измеримой потери полноты, и исключаются случаи, когда переранжирование вытесняет уже правильный первый результат. Установите 0, чтобы переранжировать каждый запрос.';
$string['rag_cap_blocked'] = 'Индексирование не выполнялось: достигнут лимит расходов RAG на этот период. Увеличьте лимит или дождитесь следующего периода. Существующий индекс остаётся без изменений.';
$string['settings:redash_export_window_days'] = 'Окно выгрузки в прошлое (дни)';
$string['settings:redash_export_window_days_desc'] = 'Насколько далеко назад уходит выгрузка, если вызывающая сторона не передала метку времени «since». Значение по умолчанию 90 дней не позволяет источнику данных, пропустившему этот параметр, выгрузить все когда-либо записанные строки; вызывающая сторона по-прежнему может запросить конкретное окно или передать since=0 для полной выгрузки. Установите 0, чтобы по умолчанию снова выгружалась вся история.';
$string['settings:redash_allow_deanonymized'] = 'Разрешить неанонимизированную выгрузку';
$string['settings:redash_allow_deanonymized_desc'] = 'По умолчанию отключено. Когда параметр отключён, запрос с anonymize=0 отклоняется, и учащиеся всегда показываются под псевдонимами. Включайте только если внешнему отчёту действительно нужны настоящие имена, и помните, что выгрузка аутентифицируется общим ключом API, а не вошедшим администратором, поэтому любой владелец ключа сможет получить эти имена. В любом случае неанонимизированные запросы записываются в журнал аудита вместе с IP-адресом запрашивающего.';

// i18n batch appended by scripts/apply_i18n.py.
$string['soapbox:present_audio'] = 'Аудиопрезентация';
$string['soapbox:present_video'] = 'Видеопрезентация';
$string['soapbox:present_target'] = 'цель {$a->min}-{$a->max} мин';
$string['soapbox:choose_topic'] = 'Выберите тему';
$string['soapbox:record_short'] = 'Записать';
$string['soapbox:my_recordings'] = 'Мои записи';
$string['soapbox:no_recordings'] = 'Записей пока нет.';
$string['soapbox:col_recorded'] = 'Записано';
$string['soapbox:col_length'] = 'Длительность';
$string['soapbox:col_type'] = 'Тип';
$string['soapbox:col_recording'] = 'Запись';
$string['soapbox:col_kept'] = 'Хранится';
$string['soapbox:col_student_link'] = 'Ссылка для студента';
$string['soapbox:view_download'] = 'Просмотр / скачать';
$string['soapbox:expired'] = 'Срок истёк';
$string['soapbox:retention_note'] = 'Записи доступны для просмотра и скачивания в течение {$a} дней, затем удаляются автоматически.';
$string['soapbox:assign_title'] = 'Задания Soapbox';
$string['soapbox:assign_single_title'] = 'Задание Soapbox';
$string['soapbox:assign_add'] = 'Добавить задание';
$string['soapbox:assign_none'] = 'Заданий Soapbox пока нет.';
$string['soapbox:assign_deleted'] = 'Задание удалено.';
$string['soapbox:assign_updated'] = 'Задание обновлено.';
$string['soapbox:assign_created'] = 'Задание создано.';
$string['soapbox:assign_delete_confirm'] = 'Удалить «{$a}» и все его записи? Это действие нельзя отменить.';
$string['soapbox:assign_edit_heading'] = 'Редактировать задание Soapbox';
$string['soapbox:assign_new_heading'] = 'Новое задание Soapbox';

$string['settings:active_learners_enabled'] = 'Показывать индикатор активных учащихся';
$string['settings:active_learners_enabled_desc'] = 'Показывает в панели чата, сколько других учащихся сейчас занимается. Это социальная подсказка, а не учебная функция, и она опрашивает сервер раз в минуту для каждой открытой панели, поэтому по умолчанию отключена. Индикатор появляется только при наличии как минимум двух других активных участников.';

$string['task:unanswered_check'] = '[[tutorshort]] проверка вопросов без ответа';
$string['settings:unanswered_check_enabled'] = 'Предупреждать, когда вопросы остаются без ответа';
$string['settings:unanswered_check_enabled_desc'] = 'Отправляет письмо получателям уведомлений о расходах, когда учащиеся задают [[tutorshort]] вопросы и не получают ответа. Детектор аномалий расходов срабатывает только при <em>росте</em> расходов: поставщик, отклоняющий все вызовы, ничего не стоит и не вызывает оповещения. Здесь отслеживается соотношение вопросов и ответов, которое падает до нуля, как только поставщик выходит из строя. Если список уведомлений пуст, используются администраторы сайта.';
$string['settings:unanswered_window_hours'] = 'Окно проверки (часы)';
$string['settings:unanswered_window_hours_desc'] = 'Насколько далеко назад смотрит каждый запуск. Короче — быстрее реакция, но нужен достаточный трафик.';
$string['settings:unanswered_min_questions'] = 'Минимум вопросов до оповещения';
$string['settings:unanswered_min_questions_desc'] = 'Курс оценивается только начиная с этого числа вопросов в окне. Ниже несколько учащихся, закрывших вкладку, выглядят как сбой.';
$string['settings:unanswered_min_answer_rate'] = 'Минимальная доля ответов';
$string['settings:unanswered_min_answer_rate_desc'] = 'Оповещать, когда доля отвеченных вопросов опускается ниже этого значения. 0,5 ловит и частичный сбой. Установите 0, чтобы отключить проверку.';
