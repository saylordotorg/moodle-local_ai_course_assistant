...
$string['pedagogy:learning_path_desc'] = 'Когда включено, SOLA добавляет визуальную панель учебного пути (кнопка "мой путь" в заголовке виджета), показывающую программу учащегося как последовательность курсов, каждый из которых можно развернуть, чтобы увидеть его цели и уровень освоения учащимся. Когда учащийся достигает порога для текущего курса (завершение или высокая доля освоенных целей), SOLA также показывает мягкий баннер "готов к следующему курсу" и упоминает об этом в беседе. Только рекомендательно; использует собственное распределение программы учащегося; ничего не делает незаметно там, где никакая программа не применяется.';
...
$string['settings:backend_retry_max_wait_desc'] = 'Верхняя граница в секундах того, как долго соблюдать заголовок Retry-After от бэкенда перед повтором. Когда бэкенд не отправляет Retry-After, SOLA вместо этого использует короткую экспоненциальную задержку. По умолчанию 5.';
...
$string['settings:zendesk_require_consent_desc'] = 'Когда включено (рекомендуется), SOLA эскалирует разговор в службу поддержки Zendesk только после того, как учащийся принял уведомление о согласии при первом запуске, в котором раскрывается, что запрос помощи человека передаёт разговор (включая имя и адрес электронной почты) в поддержку. Отключайте это только в том случае, если вы получаете это согласие иным способом; при отключённой настройке эскалации отправляются немедленно. Не действует, пока не включена эскалация Zendesk.';
...
$string['emergency:title'] = 'Экстренное управление SOLA';
$string['emergency:page_warning'] = 'Эти переключатели вступают в силу немедленно для каждого учащегося на сайте. Каждое действие записывает строку аудита. Гранулярные переключатели оставляют остальную часть SOLA работающей; главное отключение полностью удаляет виджет.';
$string['emergency:back_to_settings'] = 'Настройки SOLA';
...
$string['emergency:flag_chat_desc'] = 'Блокирует трафик чата через специальный флаг отключения (исправление v5.13). Виджет продолжает отображаться; учащиеся видят дружелюбное сообщение «SOLA приостановлена». Используйте, когда провайдер LLM ведёт себя неправильно или происходит скачок затрат.';
...
$string['emergency:flag_all_desc'] = 'Отключает весь плагин: виджет исчезает с каждой страницы, запланированные задачи останавливаются, голос очищен, RAG отключён, рассылка отключена. Самый мощный переключатель — используйте при инциденте безопасности или когда SOLA необходимо немедленно перевести в офлайн.';
$string['emergency:settings_link'] = 'Экстренное управление';
$string['emergency:settings_link_desc'] = 'Переключатели отключения по подсистемам (чат / голос / RAG / рассылка / главный) с ведением журнала аудита — веб-эквивалент <code>admin/cli/emergency_disable.php</code>. Откройте <a href="{$a}">Экстренное управление SOLA</a>.';
$string['email_unsubscribe:done_title'] = 'Отписка выполнена';
$string['email_unsubscribe:done_body'] = 'Готово — {$a->email} больше не будет получать этот тип писем от {$a->product}. Если вы передумаете, попросите администратора {$a->product} повторно включить подписку или отправьте новый запрос на подписку через страницу SOLA Recipients в панели администратора.';
...
$string['settings:cost_anomaly_multiplier_desc'] = 'Сегодняшние расходы должны превысить этот множитель × медиану за 7 дней для срабатывания оповещения. По умолчанию <code>2.0</code>. Уменьшите до <code>1.5</code> для более ранних предупреждений (больше ложных срабатываний в периоды всплеска записи). Увеличьте до <code>3.0</code>, если использование Saylor достаточно неравномерное, чтобы скачки в 2x были рутиной.';
$string['task:cost_anomaly_check'] = 'Ежедневная проверка аномалий затрат SOLA';
...
$string['task:policy_bundle_sync'] = 'Синхронизация подписанного пакета политик SOLA';
...
$string['analytics:exp_active_users'] = 'Активные пользователи SOLA';
