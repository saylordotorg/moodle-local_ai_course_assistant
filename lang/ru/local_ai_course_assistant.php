...
$string['settings:prompt_budget_mode_desc'] = 'Как определяется бюджет символов системной подсказки. <strong>Вывести из модели</strong> вычисляет, сколько места действительно даёт настроенная модель — её контекстное окно за вычетом зарезервированного ответа и истории переписки — и берёт большее из этой величины и фиксированного бюджета ниже. Это рекомендуемая настройка: фиксированное число уже дважды задавалось по измерению одной конфигурации и дважды оказывалось слишком малым в другой. <strong>Фиксированный</strong> использует только число ниже. Для собственного сервера следует вместо этого указать контекстное окно бэкенда — оно всегда имеет приоритет.';
