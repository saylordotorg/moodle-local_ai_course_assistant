...
$string['chat:consent_body'] = '{$a->product}은(는) AI 기반 학습 어시스턴트입니다. 귀하의 메시지와 {$a->product}의 응답은 {$a->institution}의 Moodle 데이터베이스에 저장되며, 가장 최근의 10개 대화 차례가 질문에 답하기 위해 승인된 AI 모델 제공업체로 전송됩니다. 개인화를 위해 귀하의 이름이 공유됩니다. 다른 식별 정보는 AI 제공업체로 전송되지 않습니다. 사람의 도움을 요청하여 질문이 에스컬레이션되면 이 대화(이름과 이메일 포함)가 저희 지원팀과 공유될 수 있습니다. 귀하는 언제든지 {$a->product}을(를) 다운로드, 삭제하거나 사용을 중단할 수 있습니다.';
...
$string['settings:current_page_content_maxchars_desc'] = 'RAG가 꺼져 있을 때 시스템 프롬프트의 "Current Page Content" 섹션으로 삽입되는 현재 페이지 텍스트의 최대 문자 수입니다. 기본값 8,000은 구조와 지침을 위한 예산을 남기면서 페이지별 질문을 잘 뒷받침합니다. (RAG가 활성화된 경우 페이지는 대신 현재 페이지에 치우친 가장 관련성 높은 자체 청크로 뒷받침되므로 이 한도는 적용되지 않습니다.) 매우 긴 페이지는 앞부분에서 이 문자 수만큼 잘리므로 극도로 긴 페이지의 끝부분은 인용되지 않을 수 있습니다. RAG를 활성화하면 이를 방지할 수 있습니다. 비용을 의식하는 사이트는 더 낮게 제한할 수 있습니다(예: 3,000-4,000). 범위 500-8,000으로 제한됩니다. <code>prompt_budget_chars</code>와 독립적입니다: 이것은 페이지 섹션만 제한하고, 예산은 전체 프롬프트를 제한합니다.';
...
$string['pedagogy:learning_path'] = '학습 경로 지도 및 다음 과정 안내 기본 활성화';
$string['pedagogy:learning_path_desc'] = '활성화하면 SOLA가 시각적 학습 경로 패널(위젯 헤더의 "내 경로" 버튼)을 추가하여 학습자의 프로그램을 일련의 과정으로 표시합니다. 각 과정은 확장하여 학습 목표와 학습자의 숙달도를 볼 수 있습니다. 학습자가 현재 과정의 기준(이수 또는 높은 비율의 목표 숙달)을 충족하면 SOLA는 부드러운 "다음 과정 준비 완료" 배너도 표시하고 대화 중에 이를 언급합니다. 권고 사항일 뿐이며, 학습자 본인의 프로그램 배정을 사용합니다. 해당하는 프로그램이 없는 경우 조용히 아무것도 하지 않습니다.';
$string['settings:learning_path_mastery_threshold'] = '학습 경로 준비 기준값 (%)';
$string['settings:learning_path_mastery_threshold_desc'] = '학습 경로 안내가 학습자를 다음 과정에 준비된 것으로 간주하기 전에 학습자가 숙달해야 하는 과정의 추적 목표 비율입니다. Moodle 과정 이수가 다른 트리거이며, 둘 중 먼저 발생하는 것이 안내를 발동시킵니다. 기본값은 80입니다.';
$string['pathpanel_title'] = '내 학습 경로';
$string['pathpanel_open'] = '내 학습 경로';
$string['pathpanel_empty'] = '이 과정에는 아직 사용 가능한 프로그램 경로가 없습니다.';
$string['path_position'] = '{$a->total}개 과정 중 {$a->index}번째';
$string['path_status_done'] = '완료';
$string['path_status_current'] = '현재 위치';
$string['path_status_upcoming'] = '예정';
$string['path_mastery_mastered'] = '숙달함';
$string['path_mastery_in_progress'] = '진행 중';
$string['path_mastery_not_started'] = '시작 안 함';
$string['path_mastery_demonstrated_elsewhere'] = '다른 과정에서 입증됨';
$string['nudge_ready_title'] = '다음으로 넘어갈 준비 완료';
$string['nudge_ready_body'] = '잘하셨어요 — {$a} 준비가 되었습니다.';
$string['nudge_view_path'] = '내 경로 보기';
$string['nudge_dismiss'] = '닫기';

// v5.10.x strings (token-aware budgeting, backend retry, self-test, deployment presets, escalation consent, privacy).
$string['settings:backend_context_tokens'] = '백엔드 컨텍스트 창(토큰)';
$string['settings:backend_context_tokens_desc'] = 'AI 백엔드의 최대 컨텍스트 길이(max_model_len)를 토큰 단위로 지정합니다. 큰 창을 가진 호스팅 모델의 경우 0으로 설정하세요(클램핑 없음). 0보다 큰 값(예: 자체 호스팅 vLLM 백엔드에서 8192)으로 설정하면 SOLA는 위의 시스템 프롬프트 문자 예산을 줄여, 토큰 밀도가 높은 언어에서도 프롬프트와 예약된 출력 및 대화 기록이 창에 맞도록 합니다. 이것이 동시 사용자 수와 어떻게 대응되는지는 Deployment Sizing 위키 페이지를 참조하세요.';
$string['settings:backend_retry_attempts'] = '백엔드 재시도 횟수';
$string['settings:backend_retry_attempts_desc'] = '학생에게 오류를 표시하기 전에 일시적인 백엔드 오류(HTTP 429 또는 503)를 재시도할 횟수입니다. 재시도는 응답 텍스트가 스트리밍되기 전에만 발생하므로 출력이 중복되지 않습니다. 부하 시 요청을 거부하는 소규모 자체 호스팅 백엔드를 위한 것입니다. 비활성화하려면 0으로 설정하세요. 기본값은 2입니다.';
$string['settings:backend_retry_max_wait'] = '백엔드 재시도 최대 대기 시간(초)';
$string['settings:backend_retry_max_wait_desc'] = '재시도 전에 백엔드의 Retry-After 헤더를 얼마나 오래 준수할지에 대한 상한값(초 단위)입니다. 백엔드가 Retry-After를 보내지 않으면 SOLA는 대신 짧은 지수 백오프를 사용합니다. 기본값은 5입니다.';
$string['prompt:truncation_hint'] = '참고: 길이 제한으로 인해 이번 차례에는 전체 강좌 내용을 검색할 수 없었습니다. 학생이 제공된 내용에서 찾을 수 없는 것을 질문하면, 강좌 전체를 검색할 수 없었다고 말하고, 강좌에 없다고 단정하기보다는 해당 주제가 다뤄지는 특정 페이지나 활동을 열어보도록 제안하세요.';
$string['selftest:title'] = '백엔드 자체 테스트';
$string['selftest:intro'] = '구성된 AI 백엔드를 실시간으로 점검합니다: 작은 채팅 왕복, 컨텍스트 창(max_model_len) 자동 감지 및 백엔드 컨텍스트 창 설정과의 비교, 시스템 프롬프트 예산 하한, 그리고 (RAG가 켜져 있을 때) 임베딩 왕복입니다. 네트워크 호출은 실행을 누를 때만 실행됩니다.';
$string['selftest:run'] = '백엔드 자체 테스트 실행';
$string['selftest:check'] = '점검';
$string['selftest:status'] = '상태';
$string['selftest:detail'] = '세부 정보';
$string['selftest:link'] = '백엔드 자체 테스트 페이지';
$string['selftest:link_desc'] = '<a href="{$a}">백엔드 자체 테스트</a> 페이지를 열어 AI 백엔드가 작동하고 올바르게 크기가 조정되었는지 확인하세요. 자체 호스팅 백엔드를 구성한 직후에 유용합니다.';
$string['profile:title'] = '배포 프리셋';
$string['profile:intro'] = '배포 유형에 권장되는 설정 묶음을 적용합니다. 값은 일반 플러그인 설정에 기록되며 이후에도 개별적으로 편집할 수 있습니다. 프리셋을 적용하면 나열된 설정을 덮어씁니다.';
$string['profile:current'] = '마지막으로 적용한 프리셋: {$a}';
$string['profile:setting'] = '설정';
$string['profile:value'] = '값';
$string['profile:self_hosted_small'] = '자체 호스팅 소형 컨텍스트(단일 GPU, 예: A30 24GB / vLLM 8K)';
$string['profile:hosted_large'] = '호스팅 대형 컨텍스트(기본값)';
$string['profile:apply_self_hosted_small'] = '자체 호스팅 소형 컨텍스트 프리셋 적용';
$string['profile:apply_hosted_large'] = '호스팅 대형 컨텍스트 기본값 적용';
$string['profile:applied'] = '{$a} 프리셋을 적용했습니다. 값은 이제 플러그인 설정에 반영되었습니다.';
...
$string['privacy:metadata:email_optout'] = '수신자별 이메일 수신 거부 환경설정(수신자가 어떤 이메일 유형의 구독을 취소했는지).';
...
$string['settings:rag_min_similarity'] = '최소 관련도(코사인)';
$string['settings:rag_min_similarity_desc'] = '질문에 대한 코사인 유사도가 이 값보다 낮은 검색된 청크를 제거하여, 주제에서 벗어났거나 정보가 부족한 질문이 항상 약한 일치로 Top-K까지 채우는 대신 더 적은(또는 0개의) 구절을 삽입하도록 합니다. 범위는 0에서 1까지이며, 0은 게이트를 비활성화합니다. 적절한 값은 임베딩 모델에 따라 다릅니다. 0.25는 text-embedding-3-small에 적합합니다. 더 엄격하게 하려면 높이고(컨텍스트는 적지만 더 주제에 부합), 더 관대하게 하려면 낮추세요.';
$string['settings:rag_currentpage_boost'] = '현재 페이지 부스트';
$string['settings:rag_currentpage_boost_desc'] = '학습자가 현재 보고 있는 페이지의 청크 관련도 점수에 추가되는 작은 보너스로, "이것을 설명해줘" 같은 질문이 점수가 비슷할 때 보이는 페이지를 선호하도록 합니다. 순서 지정만 수행합니다: 관련 없는 페이지 청크를 최소 관련도 게이트를 넘도록 강제하지 않습니다. 비활성화하려면 0으로 설정하세요.';
$string['settings:history_mode'] = '기록 선택 모드';
$string['settings:history_mode_desc'] = '지난 대화 차례가 모델로 전송되기 전에 어떻게 선택되는지를 지정합니다. <strong>의미 기반</strong>은 현재 질문과 관련된 최근 차례(그리고 항상 가장 최근의 교환)만 유지하므로, 오래되고 주제에서 벗어난 이전 차례가 비용을 부풀리거나 답변을 빗나가게 하지 않습니다. 메시지당 추가 임베딩 호출이 한 번 발생합니다. <strong>최신순</strong>은 관련성에 관계없이 마지막 "Max Conversation History" 쌍을 유지합니다(오랜 기존 동작, 추가 호출 없음). 임베딩을 사용할 수 없는 경우 의미 기반 모드는 자동으로 최신순으로 대체됩니다.';
$string['settings:history_mode_semantic'] = '의미 기반(관련된 최근 차례)';
$string['settings:history_mode_recency'] = '최신순(마지막 N쌍)';
$string['settings:history_semantic_minscore'] = '기록 관련도 하한(코사인)';
$string['settings:history_semantic_minscore_desc'] = '의미 기반 기록 모드에서는 지난 차례가 현재 질문과의 유사도가 최소한 이 값 이상일 때만 유지됩니다(가장 최근의 교환은 항상 유지됨). 범위는 0에서 1까지이며, 모델에 따라 다릅니다. 더 엄격하게 하려면 높이고(기록이 적어짐), 더 많이 유지하려면 낮추세요.';
$string['settings:history_candidates'] = '기록 후보 윈도우';
$string['settings:history_candidates_desc'] = '의미 기반 기록 모드에서는 가장 최근의 이 수만큼의 쌍만 관련성 점수가 매겨집니다(비용 제한). 이 윈도우보다 오래된 쌍은 전송되지 않습니다. 이 값을 "Max Conversation History" 이상으로 유지하세요.';
