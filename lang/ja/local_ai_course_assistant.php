...
$string['settings:prompt_budget_mode_desc'] = 'システムプロンプトの文字数予算をどのように決めるかを指定します。<strong>モデルから算出</strong>は、設定中のモデルが実際に使える余地—コンテキストウィンドウから、確保しておく応答分と会話履歴を差し引いた量—を計算し、その値と下の固定予算のうち大きい方を使います。こちらが推奨設定です。固定値はこれまで二度、ひとつの構成だけを測って決められ、二度とも別の構成では小さすぎることが判明しました。<strong>固定</strong>は下の数値だけを使います。セルフホストのバックエンドでは代わりにバックエンドのコンテキストウィンドウを設定してください。そちらが常に優先されます。';
