...
$string['pedagogy:learning_path_desc'] = 'Khi bật, SOLA thêm một bảng lộ trình học tập trực quan (nút "lộ trình của tôi" trong tiêu đề widget) hiển thị chương trình của người học dưới dạng một chuỗi các khóa học, mỗi khóa có thể mở rộng để xem mục tiêu và mức độ thành thạo của người học. Khi người học đạt được tiêu chuẩn của khóa học hiện tại (hoàn thành hoặc thành thạo một tỷ lệ cao các mục tiêu), SOLA cũng hiển thị một biểu ngữ nhẹ nhàng "sẵn sàng cho khóa học tiếp theo" và đề cập đến điều đó trong cuộc trò chuyện. Chỉ mang tính tư vấn; sử dụng phân bổ chương trình của chính người học; lặng lẽ không làm gì khi không có chương trình nào áp dụng.';
...
$string['settings:backend_retry_max_wait_desc'] = 'Giới hạn trên, tính bằng giây, về thời gian tuân theo tiêu đề Retry-After từ backend trước khi thử lại. Khi backend không gửi Retry-After, SOLA sử dụng một khoảng lùi theo cấp số nhân ngắn thay thế. Mặc định là 5.';
...
$string['emergency:title'] = 'Điều khiển khẩn cấp SOLA';
$string['emergency:page_warning'] = 'Các công tắc này có hiệu lực ngay lập tức với mọi học viên trên trang. Mỗi hành động ghi một hàng kiểm toán. Các công tắc chi tiết cho phép phần còn lại của SOLA tiếp tục chạy; công tắc tắt tổng thể xóa hoàn toàn widget.';
$string['emergency:back_to_settings'] = 'Cài đặt SOLA';
...
$string['emergency:flag_chat_desc'] = 'Chặn lưu lượng chat qua kill flag chuyên dụng (bản sửa lỗi v5.13). Widget vẫn hiển thị; học viên thấy thông báo thân thiện "SOLA đã tạm dừng". Sử dụng khi nhà cung cấp LLM gặp sự cố hoặc đang xảy ra tăng đột biến chi phí.';
...
$string['emergency:flag_all_desc'] = 'Vô hiệu hóa toàn bộ plugin: widget biến mất khỏi mọi trang, các tác vụ được lên lịch dừng lại, giọng nói bị xóa, RAG tắt, outreach tắt. Công tắc mạnh nhất — sử dụng trong trường hợp sự cố bảo mật hoặc khi SOLA cần đưa ngoại tuyến ngay lập tức.';
$string['emergency:settings_link'] = 'Điều khiển khẩn cấp';
$string['emergency:settings_link_desc'] = 'Công tắc tắt từng hệ thống con (chat / giọng nói / RAG / outreach / tổng thể) với nhật ký kiểm toán — tương đương web của <code>admin/cli/emergency_disable.php</code>. Mở <a href="{$a}">Điều khiển khẩn cấp SOLA</a>.';
$string['email_unsubscribe:done_title'] = 'Đã hủy đăng ký';
$string['email_unsubscribe:done_body'] = 'Xong — {$a->email} sẽ không còn nhận loại email này từ {$a->product} nữa. Nếu bạn thay đổi ý định, hãy nhờ quản trị viên {$a->product} kích hoạt lại đăng ký, hoặc gửi xác nhận mới qua trang quản trị SOLA Recipients.';
...
$string['settings:cost_anomaly_heading_desc'] = 'Tác vụ được lên lịch hàng ngày (<code>cost_anomaly_check</code>) so sánh chi tiêu SOLA toàn trang hôm nay với trung vị 7 ngày liên tiếp. Gửi email đến danh sách người nhận <code>spend_notify_emails</code> (mặc định về quản trị viên trang) khi hôm nay vượt quá số nhân được cấu hình × trung vị. Phát hiện ba chế độ lỗi mà các ngưỡng 80% / 95% / 100% hiện có bỏ lỡ: (1) khóa học mất kiểm soát nơi trần tuyệt đối không bị vượt nhưng một khóa đột nhiên tạo ra lưu lượng gấp 10 lần bình thường, (2) vô tình bật cấp cao cấp, (3) nhà cung cấp bị định tuyến sai. Tắt theo mặc định; tương đương trong SOLA của truy vấn Redash tại <code>.drafts/sola-redash-cost-anomaly-2026-06-09.md</code>.';
...
$string['analytics:exp_active_users'] = 'Người dùng SOLA đang hoạt động';
