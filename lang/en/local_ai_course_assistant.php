...
$string['settings:outcomes_panel_enabled_desc'] = 'Adds a "Your program outcomes" section to the learner progress panel, showing attainment against the outcomes of their degree program, read from the Outcome Map plugin.<br><br><strong>Requires Outcome Map 0.9.4 or later.</strong> Earlier versions expose pooled program attainment only through their SIS export function, which requires a site-level capability no student has and which must not be granted to students, because it can read any user\'s attainment. With an earlier version installed this setting has no effect at all: the panel is hidden from everyone rather than shown to administrators only, because a panel that appears for staff and never for the learners it describes is worse than no panel.<br><br>Off by default. Switch it on once Outcome Map 0.9.4 is in place and you are in a position to look at the result, and switch it off again in one click if it misbehaves.';
