...
$string['chat:open'] = 'Open [[tutorshort]]';
...
$string['settings:rag_min_similarity_desc'] = 'Drop retrieved chunks whose cosine similarity to the question is below this value, so an off-topic or sparse question injects fewer (or zero) passages instead of always padding to Top-K with weak matches. Range 0 to 1; 0 disables the gate. The right value depends on the embedding model, and this one is calibrated for text-embedding-3-small at 0.25. Voyage models produce a different similarity distribution, so the same number is not equivalent on voyage-4-large and has not been recalibrated -- re-run the RAG fixture benchmark against your own index before trusting it. Raise it to be stricter (less, more on-topic context), lower it to be more permissive.';
...
$string['settings:stt_selfhosted_heading_desc'] = 'Run speech-to-text on your own hardware at zero per-minute cost. Point [[tutorshort]] at any OpenAI-compatible transcription server: <code>whisper-server</code> Docker, <code>speaches</code> (faster-whisper), or <code>whisper.cpp</code> server. When a server URL is set here it becomes the default STT path; pick a paid provider in Active STT provider above to override. If the server is on a private network or plain http, also add its host to the SSRF trusted endpoints allowlist in the Security section.';
$string['settings:stt_selfhosted_url'] = 'Selfhosted STT server URL';
$string['settings:stt_selfhosted_url_desc'] = 'Base URL of the OpenAI-compatible transcription server, for example <code>http://10.0.0.5:8000</code>. [[tutorshort]] appends <code>/v1/audio/transcriptions</code> automatically; a full endpoint path is also accepted. Leave empty to disable selfhosted STT.';
...
$string['analytics:exp_heading'] = 'A/B experiment comparison';
$string['analytics:exp_desc'] = 'Compare engagement between two courses over the selected time range. Built for per-course experiments (for example the avatar animation probe): put the override in one course, leave the other as control, and read the difference here.';
$string['analytics:exp_course_a'] = 'Course A';
$string['analytics:exp_course_b'] = 'Course B';
...
$string['analytics:exp_avg_msgs_session'] = 'Avg messages per session';
$string['analytics:exp_avg_session_minutes'] = 'Avg session length (minutes)';
...
$string['emergency:flag_all_desc'] = 'Disables the entire plugin: widget gone from every page, scheduled tasks stop, voice cleared, RAG off, outreach off. The strongest switch — use for a security incident or when [[tutorshort]] must be taken offline immediately.';
$string['emergency:settings_link'] = 'Emergency controls';
$string['emergency:settings_link_desc'] = 'Per-subsystem kill switches (chat / voice / RAG / outreach / master) with audit logging — the web equivalent of <code>admin/cli/emergency_disable.php</code>. Open <a href="{$a}">[[tutorshort]] Emergency Controls</a>.';
...
$string['privacy:metadata:sbx_rec'] = 'Soapbox presentation recordings. The media object is deleted after the retention window; the transcript is kept for feedback and access requests.';
...
$string['analytics_js:ai_users'] = 'AI Users';
$string['analytics_js:non_users'] = 'Non-Users';
$string['analytics_js:thumbs_up'] = 'Thumbs Up';
$string['analytics_js:thumbs_down'] = 'Thumbs Down';
$string['analytics_js:hallucination_flags'] = 'Hallucination Flags';
$string['analytics_js:avg_star_rating'] = 'Avg Star Rating';
...
$string['analytics_js:error_loading'] = 'Error loading data';
$string['analytics_js:loading'] = 'Loading analytics...';
$string['analytics_js:no_course_data'] = 'No course data available.';
$string['analytics_js:no_unit_data'] = 'No unit data yet. Unit tracking accumulates as students use the assistant.';
$string['analytics_js:no_keyword_data'] = 'No keyword data available for this period.';
...
$string['settings:ssrf_trusted_endpoints_desc'] = 'One URL per line. Listed hosts bypass the loopback / private-IP / https-only checks in [[tutorshort]]\'s SSRF validator. Use this only for self-hosted LLMs on a network you control — for example <code>http://localhost:11434</code> for local Ollama, <code>http://10.0.0.5:8000</code> for a vLLM pod on the same VPC. Comparison matches scheme + host + port; any path is ignored. Default empty (block everything internal). Lines starting with <code>#</code> are comments.';

// xAI Realtime proxy (v3.9.13).
$string['settings:xai_proxy_url'] = 'xAI Realtime proxy URL';
$string['settings:xai_proxy_url_desc'] = 'Public wss URL of the [[tutorshort]] xAI Realtime proxy service (for example wss://voice.example.com/xai-rt/rt). When this is set together with the JWT secret, xAI voice routes through the proxy and the master xAI API key never reaches the browser. Leave empty to fall back to the direct connection (not recommended for production).';
...
$string['email_unsubscribe:done_title']   = 'Unsubscribed';
$string['email_unsubscribe:done_body']    = 'Done — {$a->email} will no longer receive this type of email from {$a->product}. If you change your mind, ask a {$a->product} administrator to re-enable the subscription, or send a new opt-in via the [[tutorshort]] Recipients admin page.';
$string['email_unsubscribe:invalid_title'] = 'Unsubscribe link no longer valid';
$string['email_unsubscribe:invalid_body']  = 'This unsubscribe link has expired or is malformed. Look for a more recent email from us, or contact a site administrator to be removed manually.';
...
$string['objectives:rebuild_links_help']    = '[[tutorshort]] links objectives that match across courses (by competency reference or title) so a learner who mastered a topic elsewhere is not re-drilled. Links rebuild automatically every night; use this button to rebuild now after editing objectives.';
...
// v5.1.7: course-picker landing for per-course pages opened without a courseid.
...
$string['pedagogy:program_path_desc']      = 'When on, [[tutorshort]] can tell a learner where the current course leads next in their program (degree or certificate) and how today\'s concepts bridge to later courses. Reads the Moodle Programs plugin (Degrees and Learn) and names a specific next course only where the program defines a prerequisite or required order; otherwise it notes the learner\'s position in the path. Advisory only — it never changes enrolment or mastery, and only ever uses the current learner\'s own program allocation. Silently does nothing where no program applies.';
$string['pedagogy:learning_path']          = 'Learning path map and next-course nudge on by default';
$string['pedagogy:learning_path_desc']     = 'When on, [[tutorshort]] adds a visual learning-path panel (a "my path" button in the widget header) showing the learner\'s program as a sequence of courses, each expandable to its objectives and the learner\'s mastery. When the learner has met the bar for the current course (completion or a high share of objectives mastered), [[tutorshort]] also surfaces a gentle "ready for the next course" banner and mentions it in conversation. Advisory only; uses the learner\'s own program allocation; silently does nothing where no program applies.';
$string['settings:learning_path_mastery_threshold']      = 'Learning-path readiness threshold (%)';
$string['settings:learning_path_mastery_threshold_desc'] = 'Percent of a course\'s tracked objectives a learner must master before the learning-path nudge treats them as ready for the next course. Moodle course completion is the other trigger; whichever happens first fires the nudge. Default 80.';
$string['pathpanel_title']                 = 'My learning path';
$string['pathpanel_open']                  = 'My learning path';
$string['pathpanel_empty']                 = 'No program path is available for this course yet.';
$string['path_position']                   = 'Course {$a->index} of {$a->total}';
$string['path_status_done']                = 'Done';
$string['path_status_current']             = 'You are here';
$string['path_status_upcoming']            = 'Upcoming';
$string['path_mastery_mastered']           = 'Mastered';
$string['path_mastery_in_progress']        = 'In progress';
$string['path_mastery_not_started']        = 'Not started';
$string['path_mastery_demonstrated_elsewhere'] = 'Shown in another course';
$string['nudge_ready_title']               = 'Ready to move on';
$string['nudge_ready_body']                = 'Nice work — you\'re ready for {$a}.';
$string['nudge_view_path']                 = 'View my path';
$string['nudge_dismiss']                   = 'Dismiss';
...
$string['settings:backend_retry_max_wait_desc'] = 'Upper bound, in seconds, on how long to honor a Retry-After header from the backend before retrying. When the backend sends no Retry-After, [[tutorshort]] uses a short exponential backoff instead. Default 5.';
...
$string['selftest:storage_title'] = 'Soapbox object storage';
$string['selftest:storage_intro']  = 'Round-trips a small test object through the configured bucket: upload, size check, download with a byte comparison, delete, then a re-check that it is gone. Each step is reported separately, because which one fails is the diagnosis. This performs real writes and deletes under a __selftest/ folder inside the configured recording prefix, so it exercises the same path as a learner recording.';
$string['selftest:storage_run']   = 'Run storage test';
...
$string['settings:prompt_proportions_heading_desc'] = 'Allocate the system prompt budget across four buckets: safety + identity, course structure, course content, and current page. Weights are percentages summing to 100. Empirically tuned defaults (10 / 10 / 40 / 40) come from the v5.6.0 weight-tuning benchmark; leaving the textarea blank uses those defaults. The automatic boost adjusts allocation per turn depending on whether a specific page is in scope.';
$string['settings:prompt_section_weights']      = 'Base section weights (JSON)';
$string['settings:prompt_section_weights_desc'] = 'Optional JSON object mapping each bucket to a percentage. Leave blank to use the benchmarked defaults (10 / 10 / 40 / 40). Example: <code>{"safety_identity": 10, "course_structure": 10, "course_content": 40, "current_page": 40}</code>. Weights must sum to 100 (±5%). <code>safety_identity</code> has a floor of 10% so jailbreak resistance and output-format markers always land in full. <code>current_page + course_content</code> must be at least 40% so the model has substantive material to ground in. Out-of-range values fall back silently to the benchmarked defaults; admins should validate by checking the prompt-debug log after saving.';
$string['settings:prompt_context_boost_mode']      = 'Context boost mode';
$string['settings:prompt_context_boost_mode_desc'] = 'Automatic adjustment that shifts weight toward the current-page section when a specific page is in scope, and toward course content when no page is selected. <strong>page_focus</strong> (default) shifts about 15 weight points. <strong>aggressive</strong> shifts 25 points and is best when learners consistently ask page-specific questions. <strong>off</strong> disables the boost; admin-set base weights apply on every turn regardless of page context.';
$string['settings:prompt_context_boost_off']         = 'Off (use base weights on every turn)';
$string['settings:prompt_context_boost_page_focus']  = 'Page focus (default, ~15 points shift)';
$string['settings:prompt_context_boost_aggressive']  = 'Aggressive (~25 points shift)';
$string['settings:prompt_section_weights_coach']      = 'Coach-mode override (JSON, optional)';
$string['settings:prompt_section_weights_coach_desc'] = 'Optional JSON object overriding the base section weights specifically during graded-quiz coach mode (when <code>quizmode=coach</code>). Useful for forcing a heavier <code>current_page</code> allocation during quizzes without affecting normal chat. Leave blank to inherit the base weights. Same validation rules as the base setting.';
...
$string['prompt_debug_view:retrieved_chunks'] = 'Retrieved chunks (RAG selection)';
$string['prompt_debug_view:retrieved_chunks_hint'] = 'The passages the retriever selected for this question, in rank order with their relevance score and source (cmid). Use this to verify the model received the best matching course content.';
...
$string['settings:soapbox_vision_model_desc'] = 'Vision-capable model for the slide-vision pass. Defaults to gpt-4o-mini.';
$string['soapbox:slide_design_note']  = 'Slide design:';
$string['soapbox:title']            = 'Soapbox';
$string['soapbox:link']             = 'Soapbox speech practice';
$string['soapbox:assign_manage']    = 'Soapbox presentation assignments';
$string['soapbox:copy_link']        = 'Student link';
...
$string['soapbox:col_oncoursepage']      = 'On course page';
$string['soapbox:oncoursepage']          = 'On course page';
$string['soapbox:addtocoursepage']       = 'Add to course page';
$string['soapbox:link_added']            = 'Added to the course page. Drag it into the section you want.';
$string['soapbox:already_on_course_page'] = 'That speech assignment is already on the course page.';
$string['soapbox:assignment_hidden']         = 'This speech assignment is not available yet. Your teacher may still be preparing it.';
$string['soapbox:assignment_hidden_preview'] = 'This assignment is hidden from students. You can see it because you can manage this course.';
...
$string['soapbox:mode_label']       = 'Presentation type';
$string['soapbox:mode_informative'] = 'Informative (explain or teach)';
$string['soapbox:mode_persuasive']  = 'Persuasive (convince)';
...
$string['soapbox:err_provider']     = 'The AI scoring service could not be reached. Please try again in a moment.';
...
$string['soapbox:level_esl_intermediate'] = 'ESL (intermediate)';
...
$string['settings:outcomes_panel_enabled'] = 'Show learners their program outcomes';
$string['settings:outcomes_panel_enabled_desc'] = 'Adds a "Your program outcomes" section to the learner progress panel, showing attainment against the outcomes of their degree program, read from the Outcome Map plugin.<br><br><strong>Requires a version of Outcome Map whose own-attainment function accepts a course id.</strong> Earlier versions expose pooled program attainment only through their SIS export function, which requires a site-level capability no student has and which must not be granted to students, because it can read any user\'s attainment. With an insufficient version installed this setting has no effect at all: the panel is hidden from everyone rather than shown to administrators only, because a panel that appears for staff and never for the learners it describes is worse than no panel. This is checked against what the installed function declares, not against a version number.<br><br>Off by default. Switch it on once a sufficient Outcome Map is in place and you are in a position to look at the result, and switch it off again in one click if it misbehaves.';
$string['settings:outcomes_benchmark_default']   = 'Outcome benchmark (percent)';
$string['settings:outcomes_benchmark_default_desc'] = 'The institution-set benchmark for the outcomes report: a student meets an outcome when their mastery is at or above this percentage. This is the reporting standard (distinct from the coaching mastery threshold). Default 70.';
$string['outcomes:panel_heading'] = 'Your program outcomes';
$string['outcomes:panel_intro'] = 'These are the learning outcomes for your program, across every course that contributes to them. They are separate from the objectives for this course, and they update as you complete assessed work elsewhere.';
$string['outcomes:no_percentage_yet'] = 'No result yet';
$string['outcomes:label_insufficient_evidence'] = 'No result yet';
$string['outcomes:label_calculation_pending'] = 'Being worked out';
$string['outcomes:label_stale'] = 'Out of date';
$string['outcomes:label_not_released'] = 'Not published yet';
$string['outcomes:label_not_assessed'] = 'Not assessed';
$string['outcomes:state_insufficient_evidence'] = 'Not enough assessed work yet. This outcome needs more graded questions across your courses before a result can be worked out. It does not mean you scored zero.';
$string['outcomes:state_calculation_pending'] = 'Your result is being worked out and should appear shortly.';
...
$string['outcomes:title']       = 'Outcomes report';
$string['outcomes:intro']       = 'For each course outcome, the benchmark and the percentage of assessed students who met or exceeded it. The site benchmark is {$a}%.';
$string['outcomes:none']        = 'No outcomes are defined for this course yet.';
$string['outcomes:export']      = 'Export CSV';
$string['outcomes:col_outcome'] = 'Outcome';
$string['outcomes:col_benchmark'] = 'Benchmark';
$string['outcomes:col_assessed'] = 'Students assessed';
$string['outcomes:col_met']     = 'Met benchmark';
$string['outcomes:col_pct']     = 'Percent met';
$string['outcomes:footnote']    = 'Assessed students are those with at least one attempt on the outcome. Achievement is reported in aggregate; individual progression is never gated on any single outcome.';
$string['outcomes:navlink']     = 'Outcomes report';
...
$string['settings:premium_escalation_heading_desc'] = 'Optional per-turn routing to a premium model (Claude Opus 4.8 by default) for prompts where the workhorse chat tier visibly struggles — typically multi-step math, CS, and science reasoning. Resolved by the 2026-06-09 A.10 bake-off: Opus 4.8 won at 14.97/15 vs gpt-4o\'s 12.68/15 on hard prompts. Two trigger paths: regex matches on the user message, OR a course allowlist that escalates every turn. Off by default. At ~5% escalation, expect ~$700/month at 100k [[unishort]] MAU on top of the baseline chat spend.';
...
$string['settings:embed_input_type_mode_desc'] = 'Voyage embeddings only. Voyage projects text differently depending on whether it is being indexed or searched for. <b>Asymmetric</b> sends <code>input_type=query</code> on the retrieval call and <code>input_type=document</code> on the index call, which is what Voyage intends. <b>Shared space</b> sends <code>input_type=document</code> for both. Asymmetric retrieves better on this corpus, measured twice: +30.8 pp recall on voyage-3.5 (2026-08-21) and +24.1 pp on voyage-4-large (2026-09-10), both on production-shaped fixture sets. Pick asymmetric unless you are mid-migration; shared is migration insurance only, because projecting both sides the same way lets you change the query model later without re-embedding the corpus. Asymmetric is the shipped default from 7.4.5 onward. Sites installed before 7.4.5 keep whatever they had, so if this site predates 7.4.5 and has never touched this setting, check it. Switching needs no re-index: only the query-side projection changes. <b>This is not asymmetric model pairing</b> (indexing with a larger Voyage model than you query with). Pairing is governed by the embedding query model setting, not this one, and it did not reproduce a gain here: a wash for voyage-4-lite, 1.0 pp worse for voyage-4.';
$string['settings:embed_input_type_shared'] = 'Shared space (migration insurance; retrieves worse)';
$string['settings:embed_input_type_asymmetric'] = 'Asymmetric query/document (recommended)';
