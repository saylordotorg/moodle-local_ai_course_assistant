...
$string['settings:outcomes_panel_enabled_desc'] = 'Adds a "Your program outcomes" section to the learner progress panel, showing attainment against the outcomes of their degree program, read from the Outcome Map plugin.<br><br><strong>Off by default, and it will not work for learners yet.</strong> The only way to read pooled program attainment from Outcome Map is its SIS export function, which requires a site-level capability no student has and which must not be granted to students, because it can read any user\'s attainment. Until Outcome Map offers a learner-safe equivalent, switching this on shows the panel to administrators only. It is present so the feature can be enabled without a code deploy once that API exists, and disabled again in one click if it misbehaves.';
