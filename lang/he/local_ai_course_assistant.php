...
$string['pedagogy:learning_path_desc'] = 'כשהאפשרות מופעלת, SOLA מוסיף פאנל חזותי של מסלול הלמידה (כפתור "המסלול שלי" בכותרת הווידג\'ט) המציג את התוכנית של הלומד כרצף של קורסים, שכל אחד מהם ניתן להרחבה כדי להציג את היעדים שלו ואת רמת השליטה של הלומד. כאשר הלומד עומד ברף של הקורס הנוכחי (השלמה או שיעור גבוה של יעדים שנשלטו), SOLA מציג גם באנר עדין של "מוכן לקורס הבא" ומזכיר זאת בשיחה. ייעוצי בלבד; משתמש בהקצאת התוכנית של הלומד עצמו; אינו עושה דבר בשקט כאשר אין תוכנית רלוונטית.';
...
$string['settings:backend_retry_max_wait_desc'] = 'גבול עליון, בשניות, לכמה זמן לכבד כותרת Retry-After מה-backend לפני ניסיון חוזר. כאשר ה-backend אינו שולח Retry-After, SOLA משתמשת במקום זאת בהשהיה אקספוננציאלית קצרה. ברירת מחדל 5.';
...
$string['settings:zendesk_require_consent_desc'] = 'כאשר מופעל (מומלץ), SOLA מעבירה שיחה לדלפק התמיכה של Zendesk רק לאחר שהלומד קיבל את הודעת ההסכמה של ההפעלה הראשונה, המגלה שבקשת עזרה אנושית משתפת את השיחה (כולל שם ודוא"ל) עם התמיכה. כבה זאת רק אם אתה משיג הסכמה זו בדרך אחרת; כשהוא כבוי, ההעברות נשלחות מיד. אין לכך השפעה אלא אם העברה ל-Zendesk מופעלת.';
...
$string['settings:stt_selfhosted_heading_desc'] = 'הרץ המרת דיבור לטקסט על החומרה שלך בעלות אפס לדקה. כוון את SOLA לכל שרת תמלול תואם OpenAI: Docker של <code>whisper-server</code>, <code>speaches</code> (faster-whisper) או שרת <code>whisper.cpp</code>. כאשר מוגדרת URL שרת כאן היא הופכת לנתיב STT ברירת המחדל; בחר ספק בתשלום ב-ספק STT פעיל למעלה כדי לעקוף. אם השרת נמצא ברשת פרטית או בhttp רגיל, הוסף גם את המארח שלו לרשימת endpoint אמינים של SSRF בסעיף האבטחה.';
$string['settings:stt_selfhosted_url'] = 'URL שרת STT באירוח עצמי';
$string['settings:stt_selfhosted_url_desc'] = 'URL בסיס של שרת תמלול תואם OpenAI, לדוגמה <code>http://10.0.0.5:8000</code>. SOLA מוסיף <code>/v1/audio/transcriptions</code> אוטומטית; גם נתיב endpoint מלא מתקבל. השאר ריק כדי לנטרל STT באירוח עצמי.';
...
$string['emergency:title'] = 'בקרות חירום של SOLA';
$string['emergency:page_warning'] = 'מתגים אלו נכנסים לתוקף מיד עבור כל לומד באתר. כל פעולה כותבת שורת ביקורת. מתגים גרנולריים משאירים את שאר SOLA פועל; ה-kill הראשי מסיר את הווידג\'ט לחלוטין.';
$string['emergency:back_to_settings'] = 'הגדרות SOLA';
...
$string['emergency:flag_chat_desc'] = 'חוסם תעבורת צ\'אט דרך דגל ה-kill הייעודי (תיקון v5.13). הווידג\'ט ממשיך להיות מוצג; הלומדים רואים את ההודעה הידידותית "SOLA בהפסקה". השתמש כאשר ספק LLM מתנהג רע או שקיים עלייה בעלויות.';
...
$string['emergency:flag_all_desc'] = 'משבית את כל הפלאגין: הווידג\'ט נעלם מכל דף, משימות מתוזמנות נעצרות, קול מנוקה, RAG כבוי, פנייה כבויה. המתג החזק ביותר — השתמש עבור אירוע אבטחה או כאשר SOLA חייב להיות מוסר מהאוויר מיד.';
$string['emergency:settings_link'] = 'בקרות חירום';
$string['emergency:settings_link_desc'] = 'מתגי kill לפי תת-מערכת (צ\'אט / קול / RAG / פנייה / ראשי) עם תיעוד ביקורת — שקול הווב של <code>admin/cli/emergency_disable.php</code>. פתח <a href="{$a}">בקרות חירום של SOLA</a>.';
$string['email_unsubscribe:done_title'] = 'בוטל הרישום';
$string['email_unsubscribe:done_body'] = 'סיימנו — {$a->email} לא יקבל יותר סוג זה של דוא"ל מ-{$a->product}. אם תחליף דעתך, בקש ממנהל {$a->product} להפעיל מחדש את המינוי, או שלח הצטרפות חדשה דרך דף ניהול נמעני SOLA.';
...
$string['analytics:exp_active_users'] = 'משתמשי SOLA פעילים';
