...
$string['settings:backend_context_tokens_desc'] = 'La longitud máxima de contexto (max_model_len) de tu backend de IA, en tokens. Establécela en 0 para modelos alojados con una ventana amplia (sin recorte). Cuando se establece por encima de 0 (por ejemplo, 8192 en un backend vLLM autoalojado), SOLA reduce el presupuesto de caracteres del prompt del sistema indicado arriba para que el prompt más la salida reservada y el historial de conversación quepan en la ventana, incluso en idiomas con alta densidad de tokens. Consulta la página wiki de Dimensionamiento de Despliegue para ver cómo se relaciona esto con los usuarios concurrentes.';
...
$string['settings:backend_retry_max_wait_desc'] = 'Límite superior, en segundos, del tiempo que se respeta una cabecera Retry-After del backend antes de reintentar. Cuando el backend no envía Retry-After, SOLA utiliza en su lugar un breve retroceso exponencial. Predeterminado: 5.';
...
$string['settings:stt_selfhosted_heading_desc'] = 'Ejecute la conversión de voz a texto en su propio hardware a coste cero por minuto. Apunte SOLA a cualquier servidor de transcripción compatible con OpenAI: Docker de <code>whisper-server</code>, <code>speaches</code> (faster-whisper) o servidor <code>whisper.cpp</code>. Cuando se establece una URL de servidor aquí, se convierte en la ruta STT predeterminada; seleccione un proveedor de pago en Proveedor STT activo arriba para anularlo. Si el servidor está en una red privada o http simple, también añada su host a la lista de endpoints de confianza para SSRF en la sección de Seguridad.';
$string['settings:stt_selfhosted_url'] = 'URL del servidor STT autoalojado';
$string['settings:stt_selfhosted_url_desc'] = 'URL base del servidor de transcripción compatible con OpenAI, por ejemplo <code>http://10.0.0.5:8000</code>. SOLA añade <code>/v1/audio/transcriptions</code> automáticamente; también se acepta una ruta de endpoint completa. Déjela vacía para desactivar el STT autoalojado.';
...
$string['emergency:title'] = 'Controles de emergencia de SOLA';
$string['emergency:page_warning'] = 'Estos interruptores surten efecto de inmediato para todos los estudiantes del sitio. Cada acción escribe una fila de auditoría. Los interruptores detallados dejan el resto de SOLA en funcionamiento; el kill maestro elimina el widget completamente.';
$string['emergency:back_to_settings'] = 'Configuración de SOLA';
...
$string['emergency:flag_chat_desc'] = 'Bloquea el tráfico de chat mediante el indicador de kill dedicado (corrección v5.13). El widget sigue renderizándose; los estudiantes ven el mensaje amigable "SOLA pausado". Úselo cuando un proveedor de LLM se comporta mal o hay un pico de costes en curso.';
...
$string['emergency:flag_all_desc'] = 'Desactiva el plugin completo: el widget desaparece de todas las páginas, las tareas programadas se detienen, la voz se borra, RAG desactivado, difusión desactivada. El interruptor más potente — úselo para un incidente de seguridad o cuando SOLA deba desconectarse de inmediato.';
$string['emergency:settings_link'] = 'Controles de emergencia';
$string['emergency:settings_link_desc'] = 'Interruptores de kill por subsistema (chat / voz / RAG / difusión / maestro) con registro de auditoría — el equivalente web de <code>admin/cli/emergency_disable.php</code>. Abrir <a href="{$a}">Controles de emergencia de SOLA</a>.';
$string['email_unsubscribe:done_title'] = 'Cancelada la suscripción';
$string['email_unsubscribe:done_body'] = 'Listo — {$a->email} ya no recibirá este tipo de correo electrónico de {$a->product}. Si cambia de opinión, pida a un administrador de {$a->product} que reactive la suscripción, o envíe un nuevo opt-in a través de la página de administración de Destinatarios de SOLA.';
...
$string['settings:cost_anomaly_heading_desc'] = 'Tarea programada diaria (<code>cost_anomaly_check</code>) que compara el gasto de SOLA del sitio de hoy con la mediana móvil de 7 días. Envía un correo electrónico a la lista de destinatarios de <code>spend_notify_emails</code> (cayendo a los administradores del sitio) cuando hoy supera el multiplicador configurado × la mediana. Detecta tres modos de fallo que los umbrales del 80% / 95% / 100% del límite de gasto existente no detectan: (1) curso desbocado donde no se supera el techo absoluto pero un único curso genera de repente 10x su volumen habitual, (2) activación accidental del tier premium, (3) enrutamiento incorrecto del proveedor. Desactivado por defecto; el equivalente dentro de SOLA de la consulta Redash en <code>.drafts/sola-redash-cost-anomaly-2026-06-09.md</code>.';
...
$string['analytics:exp_active_users'] = 'Usuarios activos de SOLA';
