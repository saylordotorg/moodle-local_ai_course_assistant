...
$string['pedagogy:crossmastery_desc'] = 'Quando ativado, o SOLA reconhece quando um aluno já dominou um objetivo em outro curso (correspondido por referência de competência ou por título) e reconhece essa competência prévia em vez de exercitá-la novamente. Requer o acompanhamento de domínio; cursos sem objetivos têm um comportamento alternativo seguro. Apenas consultivo: nunca altera a pontuação de domínio armazenada do aluno em nenhum curso.';
...
$string['settings:backend_retry_max_wait_desc'] = 'Limite superior, em segundos, de quanto tempo honrar um cabeçalho Retry-After do backend antes de repetir. Quando o backend não envia Retry-After, o SOLA usa, em vez disso, um curto recuo exponencial. Padrão 5.';
...
$string['settings:stt_selfhosted_heading_desc'] = 'Execute speech-to-text em seu próprio hardware sem custo por minuto. Aponte o SOLA para qualquer servidor de transcrição compatível com OpenAI: Docker <code>whisper-server</code>, <code>speaches</code> (faster-whisper) ou servidor <code>whisper.cpp</code>. Quando uma URL de servidor é definida aqui, ela se torna o caminho STT padrão; escolha um provedor pago em Provedor STT ativo acima para substituir. Se o servidor estiver em uma rede privada ou plain http, adicione também seu host à lista de permissões de endpoints confiáveis SSRF na seção Segurança.';
$string['settings:stt_selfhosted_url'] = 'URL do servidor STT auto-hospedado';
$string['settings:stt_selfhosted_url_desc'] = 'URL base do servidor de transcrição compatível com OpenAI, por exemplo <code>http://10.0.0.5:8000</code>. O SOLA acrescenta <code>/v1/audio/transcriptions</code> automaticamente; um caminho de endpoint completo também é aceito. Deixe vazio para desativar o STT auto-hospedado.';
...
$string['emergency:title'] = 'Controles de emergência do SOLA';
$string['emergency:page_warning'] = 'Esses switches têm efeito imediato para todos os alunos do site. Cada ação grava uma linha de auditoria. Switches granulares deixam o restante do SOLA funcionando; o kill master remove o widget completamente.';
$string['emergency:back_to_settings'] = 'Configurações do SOLA';
...
$string['emergency:flag_chat_desc'] = 'Bloqueia o tráfego de chat via flag de kill dedicado (correção v5.13). O widget continua renderizando; os alunos veem a mensagem amigável "SOLA em pausa". Use quando um provedor LLM estiver se comportando mal ou um pico de custos estiver em andamento.';
...
$string['emergency:flag_all_desc'] = 'Desativa o plugin inteiro: widget removido de todas as páginas, tarefas agendadas param, voz limpa, RAG desativado, comunicações desativadas. O switch mais forte — use para incidente de segurança ou quando o SOLA precisar ser retirado do ar imediatamente.';
$string['emergency:settings_link'] = 'Controles de emergência';
$string['emergency:settings_link_desc'] = 'Switches de kill por subsistema (chat / voz / RAG / comunicações / master) com registro de auditoria — o equivalente web de <code>admin/cli/emergency_disable.php</code>. Abra <a href="{$a}">Controles de emergência do SOLA</a>.';
$string['email_unsubscribe:done_title'] = 'Descadastrado';
$string['email_unsubscribe:done_body'] = 'Pronto — {$a->email} não receberá mais este tipo de e-mail de {$a->product}. Se mudar de ideia, peça a um administrador do {$a->product} para reativar a assinatura, ou envie um novo opt-in pela página SOLA Recipients no painel de administração.';
...
$string['settings:cost_anomaly_heading_desc'] = 'Tarefa agendada diária (<code>cost_anomaly_check</code>) que compara os gastos SOLA do site de hoje com a mediana dos 7 dias anteriores. Envia e-mail para a lista de destinatários <code>spend_notify_emails</code> (com fallback para administradores do site) quando hoje supera o multiplicador configurado × mediana. Detecta três modos de falha que os limiares 80% / 95% / 100% de limite de gastos existentes não capturam: (1) curso descontrolado onde o teto absoluto não é cruzado mas um único curso de repente gera 10x seu volume habitual, (2) ativação acidental do nível premium, (3) roteamento incorreto de provedor. Desativado por padrão; o equivalente interno do SOLA da consulta Redash em <code>.drafts/sola-redash-cost-anomaly-2026-06-09.md</code>.';
...
$string['settings:cost_anomaly_multiplier_desc'] = 'Os gastos de hoje devem superar este multiplicador × a mediana de 7 dias para acionar um alerta. Padrão <code>2.0</code>. Reduza para <code>1.5</code> para avisos mais antecipados (mais falsos positivos durante picos de matrícula). Aumente para <code>3.0</code> se o uso do Saylor for irregular o suficiente para que picos de 2x sejam rotineiros.';
$string['task:cost_anomaly_check'] = 'Verificação diária de anomalias de custo do SOLA';
...
$string['task:policy_bundle_sync'] = 'Sincronização do pacote de políticas assinado do SOLA';
...
$string['analytics:exp_active_users'] = 'Usuários ativos do SOLA';
